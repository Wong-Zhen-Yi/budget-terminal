...
class CacheManager:
    ...
    def __init__(self, db_path: Any=None) -> None:
        ...
        self.db_path = str(user_data_path('budget_cache.db')) if self._uses_default_db else str(db_path)
        self._init_db()
    ...
    @contextmanager
    def _connect(self, *, configure_journal: bool = False) -> Any:
        """Open a consistently configured SQLite connection."""
        conn = sqlite3.connect(self.db_path, timeout=2.0)
        try:
            conn.execute('PRAGMA busy_timeout = 2000')
            if configure_journal and self._uses_default_db:

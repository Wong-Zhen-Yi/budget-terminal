...
def _runtime_root() -> Path:
    """Return a writable application root without relying on the install folder."""
    try:
        return user_data_dir()
    except Exception:
        return Path(tempfile.gettempdir()) / 'BudgetTerminal'


def error_log_path() -> Path:
    """Return the per-user persistent error log path."""
...
def configure_error_logging() -> Path:
    """Configure persistent error logging without making it launch-critical."""
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            'Persistent error logging is unavailable at %s; continuing with console logging.',
            path,
            exc_info=True,
        )
    return path

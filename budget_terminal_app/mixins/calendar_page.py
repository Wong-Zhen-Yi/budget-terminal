...
class CalendarPageMixin:
    ...
    def _p7_restore_session_snapshot(self, snapshot: Any, *, restore_saved_month: bool = True) -> bool:
        """Restore cached Calendar data, optionally preserving its saved month view."""
        ...
        try:
            ...
            year = month = 0
        if restore_saved_month and year > 0 and 1 <= month <= 12:
            self._p7_year = year
    ...
    def _p7_restore_startup_session(self, snapshot: Any) -> None:
        self._p7_restore_session_snapshot(snapshot, restore_saved_month=False)
    ...
    def _p7_save_splitter_sizes(self, *_: Any) -> None:
        ...
        if not hasattr(self, 'p7_details_splitter'):
            return
        if self.p7_details_splitter.orientation() != Qt.Orientation.Horizontal:
            return
    ...
    def _p7_monthly_view_width(self) -> int:
        """Return the available width for the monthly Calendar tab."""
        scroll = getattr(self, 'p7_calendar_scroll', None)
        if scroll is not None:
            viewport = scroll.viewport()
            if viewport is not None and viewport.width() > 0:
                return int(viewport.width())
        tab = getattr(self, 'p7_calendar_tab', None)
        if tab is not None and tab.width() > 0:
            return int(tab.width())
        page = getattr(self, 'page7', None)
        if page is not None and page.width() > 0:
            return int(page.width())
        return int(self.width()) if hasattr(self, 'width') else 0

    def _p7_calendar_cell_style(self, bg: str, border: str, *, empty: bool = False) -> str:
        """Return compact-aware styling for a monthly calendar cell."""
        compact = bool(getattr(self, '_p7_monthly_compact', False))
        very_compact = bool(getattr(self, '_p7_monthly_very_compact', False))
        padding = 2 if compact else 3
        min_height = 48 if very_compact else 56 if compact else 70
        font_size = 9 if very_compact else 10
        if empty:
            font_size = 12 if compact else 18
        return (
            f'QLabel {{ background: {bg}; border: 1px solid {border}; border-radius: 4px; '
            f'padding: {padding}px; font-size: {font_size}px; min-height: {min_height}px; }}'
        )

    def _p7_apply_monthly_grid_shell_style(self) -> None:
        """Apply compact-aware styling around the monthly grid."""
        compact = bool(getattr(self, '_p7_monthly_compact', False))
        very_compact = bool(getattr(self, '_p7_monthly_very_compact', False))
        if hasattr(self, 'p7_grid'):
            self.p7_grid.setSpacing(1 if compact else 2)
        font_size = 11 if very_compact else 12 if compact else 16
        padding = 1 if compact else 2
        for label in getattr(self, 'p7_weekday_labels', []):
            label.setStyleSheet(
                f'font-weight: bold; color: #888; font-size: {font_size}px; padding: {padding}px;'
            )

    def _p7_apply_monthly_responsive_layout(self) -> None:
        """Adapt the monthly Calendar tab for narrow app windows."""
        if not hasattr(self, 'p7_calendar_scroll'):
            return
        width = self._p7_monthly_view_width()
        compact = width <= _P7_MONTHLY_COMPACT_WIDTH
        very_compact = width <= _P7_MONTHLY_VERY_COMPACT_WIDTH
        previous_mode = (
            getattr(self, '_p7_monthly_compact', None),
            getattr(self, '_p7_monthly_very_compact', None),
        )
        self._p7_monthly_compact = compact
        self._p7_monthly_very_compact = very_compact

        primary_row = getattr(self, 'p7_header_primary_layout', None)
        secondary_row = getattr(self, 'p7_header_secondary_layout', None)
        title_lbl = getattr(self, 'p7_title_label', None)
        nav_widget = getattr(self, 'p7_nav_widget', None)
        actions_widget = getattr(self, 'p7_actions_widget', None)
        secondary_widget = getattr(self, 'p7_header_secondary_widget', None)
        header_mode_changed = getattr(self, '_p7_header_compact_mode', None) != compact
        if primary_row is not None and secondary_row is not None and header_mode_changed:
            for row_layout in (primary_row, secondary_row):
                while row_layout.count():
                    row_layout.takeAt(0)
            if compact:
                if nav_widget is not None:
                    primary_row.addWidget(nav_widget)
                primary_row.addStretch()
                if title_lbl is not None:
                    primary_row.addWidget(title_lbl)
                if actions_widget is not None:
                    secondary_row.addWidget(actions_widget)
                secondary_row.addStretch()
                if secondary_widget is not None:
                    secondary_widget.setVisible(True)
            else:
                if title_lbl is not None:
                    primary_row.addWidget(title_lbl)
                primary_row.addStretch()
                if nav_widget is not None:
                    primary_row.addWidget(nav_widget)
                if actions_widget is not None:
                    primary_row.addWidget(actions_widget)
                if secondary_widget is not None:
                    secondary_widget.setVisible(False)
            self._p7_header_compact_mode = compact

        if hasattr(self, 'p7_today_btn'):
            self.p7_today_btn.setText('Today')
            self.p7_today_btn.setToolTip('Jump to present')
        if hasattr(self, 'p7_export_llm_btn'):
            self.p7_export_llm_btn.setText('Export')
            self.p7_export_llm_btn.setToolTip('Export current month for LLM')
        if hasattr(self, 'p7_tz_combo'):
            self.p7_tz_combo.setMinimumWidth(86 if very_compact else 96)
            self.p7_tz_combo.setMaximumWidth(118 if compact else 150)

        splitter = getattr(self, 'p7_details_splitter', None)
        if splitter is not None:
            if compact:
                if splitter.orientation() == Qt.Orientation.Horizontal:
                    sizes = [int(size) for size in splitter.sizes() if int(size) > 0]
                    if len(sizes) == _P7_SPLITTER_PANE_COUNT:
                        self._p7_desktop_splitter_sizes = sizes
                    splitter.setOrientation(Qt.Orientation.Vertical)
                    splitter.setSizes([160] * _P7_SPLITTER_PANE_COUNT)
            else:
                if splitter.orientation() == Qt.Orientation.Vertical:
                    splitter.setOrientation(Qt.Orientation.Horizontal)
                    sizes = list(getattr(self, '_p7_desktop_splitter_sizes', []) or [])
                    if len(sizes) == _P7_SPLITTER_PANE_COUNT:
        ...
        mode_changed = previous_mode != (compact, very_compact)
        if (
            mode_changed
            and getattr(self, '_p7_monthly_rendered', False)
            and hasattr(self, 'p7_month_label')
            and getattr(self, '_p7_year', None)
        ):
            self._p7_render_month()
    ...
    def init_page7(self) -> None:
        ...
        calendar_tab_layout = QVBoxLayout(self.p7_calendar_tab)
        calendar_tab_layout.setContentsMargins(0, 0, 0, 0)
        calendar_tab_layout.setSpacing(0)
        self.p7_calendar_scroll = QScrollArea()
        self.p7_calendar_scroll.setWidgetResizable(True)
        self.p7_calendar_scroll.setFrameShape(QFrame.Shape.NoFrame)
        self.p7_calendar_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        self.p7_calendar_scroll.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded)
        self.p7_calendar_scroll.setStyleSheet('background: transparent;')
        self.p7_calendar_content = QWidget()
        self.p7_calendar_content.setMinimumWidth(0)
        self.p7_calendar_content.setStyleSheet('background: transparent;')
        layout = QVBoxLayout(self.p7_calendar_content)
        layout.setContentsMargins(8, 6, 8, 6)
        layout.setSpacing(4)
        self.p7_calendar_scroll.setWidget(self.p7_calendar_content)
        calendar_tab_layout.addWidget(self.p7_calendar_scroll)

        self.p7_header_widget = QWidget()
        self.p7_header_widget.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
        self.p7_header_layout = QVBoxLayout(self.p7_header_widget)
        self.p7_header_layout.setContentsMargins(0, 0, 0, 0)
        self.p7_header_layout.setSpacing(4)
        self.p7_header_primary_widget = QWidget()
        self.p7_header_primary_layout = QHBoxLayout(self.p7_header_primary_widget)
        self.p7_header_primary_layout.setContentsMargins(0, 0, 0, 0)
        self.p7_header_primary_layout.setSpacing(6)
        self.p7_header_secondary_widget = QWidget()
        self.p7_header_secondary_layout = QHBoxLayout(self.p7_header_secondary_widget)
        self.p7_header_secondary_layout.setContentsMargins(0, 0, 0, 0)
        self.p7_header_secondary_layout.setSpacing(6)
        self.p7_header_layout.addWidget(self.p7_header_primary_widget)
        self.p7_header_layout.addWidget(self.p7_header_secondary_widget)
        self.p7_title_label = QLabel('<b>Calendar</b>')
        self.p7_title_label.setStyleSheet('font-size: 18px; color: white;')

        self.p7_nav_widget = QWidget()
        self.p7_nav_layout = QHBoxLayout(self.p7_nav_widget)
        self.p7_nav_layout.setContentsMargins(0, 0, 0, 0)
        self.p7_nav_layout.setSpacing(4)
        ...
        self.p7_nav_layout.addWidget(self.p7_prev_btn)
        self.p7_nav_layout.addWidget(self.p7_month_label)
        self.p7_nav_layout.addWidget(self.p7_next_btn)

        self.p7_actions_widget = QWidget()
        self.p7_actions_layout = QHBoxLayout(self.p7_actions_widget)
        self.p7_actions_layout.setContentsMargins(0, 0, 0, 0)
        self.p7_actions_layout.setSpacing(6)
        self.p7_today_btn = QPushButton('Today')
        self.p7_today_btn.setFixedHeight(26)
        self.p7_today_btn.setToolTip('Jump to present')
        self.p7_today_btn.clicked.connect(self._p7_jump_to_present)
        self.p7_tz_combo = QComboBox()
        self.p7_tz_combo.setMinimumWidth(96)
        self.p7_tz_combo.setMaximumWidth(150)
        self.p7_tz_combo.setSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed)
        ...
        self.p7_ref_tz_label = QLabel('Ref TZ')
        self.p7_export_llm_btn = QPushButton('Export')
        self.p7_export_llm_btn.setToolTip('Export current month for LLM')
        self.p7_export_llm_btn.clicked.connect(self._p7_export_for_llm)
        self.p7_actions_layout.addWidget(self.p7_today_btn)
        self.p7_actions_layout.addWidget(self.p7_ref_tz_label)
        self.p7_actions_layout.addWidget(self.p7_tz_combo)
        self.p7_actions_layout.addWidget(self.p7_export_llm_btn)
        self.p7_header_primary_layout.addWidget(self.p7_title_label)
        self.p7_header_primary_layout.addStretch()
        self.p7_header_primary_layout.addWidget(self.p7_nav_widget)
        self.p7_header_primary_layout.addWidget(self.p7_actions_widget)
        self.p7_header_secondary_widget.setVisible(False)
        layout.addWidget(self.p7_header_widget)
        self.p7_grid = QGridLayout()
        self.p7_grid.setSpacing(2)
        self.p7_weekday_labels = []
        for col, day_name in enumerate(['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']):
            ...
            self.p7_grid.setColumnStretch(col, 1)
            self.p7_weekday_labels.append(lbl)
        self.p7_day_cells = []
        for row in range(6):
            row_cells = []
            for col in range(7):
                ...
                cell.setStyleSheet(self._p7_calendar_cell_style('#1a1a2e', '#2a2a4a'))
                self.p7_grid.addWidget(cell, row + 1, col)
                row_cells.append(cell)
            self.p7_day_cells.append(row_cells)
            self.p7_grid.setRowStretch(row + 1, 1)
        ...
        self.p7_details_splitter.setChildrenCollapsible(False)
        self.p7_details_splitter.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
    ...
    def _p7_render_month(self) -> None:
        ...
        compact = bool(getattr(self, '_p7_monthly_compact', False))
        very_compact = bool(getattr(self, '_p7_monthly_very_compact', False))
        day_font_size = 12 if very_compact else 13 if compact else 15
        ...
        for row in range(6):
            for col in range(7):
                ...
                if idx >= len(month_days) or month_days[idx] == 0:
                    cell.setText('')
                    cell.setStyleSheet(self._p7_calendar_cell_style('#12122a', '#1a1a2e', empty=True))
                    continue
                day = month_days[idx]
                ...
                parts = [f"<span style='font-size:{day_font_size}px; font-weight:bold; color:{day_color};'>{day}</span>"]
                events = date_events.get(day, [])
                for label, suffix, color in events[:event_limit]:
                    tag = f'{label} {suffix}'.strip()
                    parts.append(f"<span style='color:{color}; font-size:{event_font_size}px;'>{tag}</span>")
                remaining_count = len(events) - event_limit
                if remaining_count > 0:
                    parts.append(
                        f"<span style='color:#8b95aa; font-size:{event_font_size}px;'>+{remaining_count} more</span>"
                    )
                cell.setText('<br>'.join(parts))
                cell.setStyleSheet(self._p7_calendar_cell_style(bg, border))
        economic_events = []
        company_events = []

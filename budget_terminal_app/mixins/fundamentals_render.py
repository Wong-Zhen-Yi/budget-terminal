...
class FundamentalsRenderMixin:
    ...
    def _p2_compute_headline_metrics(self, data: Any) -> dict[str, Any]:
        """Compute the ten headline Fundamentals metrics for one payload."""
        payload = data if isinstance(data, dict) else {}
        info = payload.get('info') if isinstance(payload.get('info'), dict) else {}

        def sg(key: Any) -> Any:
            """Return one info field, treating 'N/A' as missing."""
            v = info.get(key)
            return None if v is None or v == 'N/A' else v

        def reported_flow(family: str, aliases: list[str]) -> float | None:
            """Prefer a trailing four-quarter sum, falling back to the latest annual value."""
            quarterly = self._p2_statement_frame(payload, family, 'quarterly')
            ...
            annual = self._p2_statement_frame(payload, family, 'annual')
        ...
        if fcf is None:
            fcf = sg('freeCashflow')
        cash_values, _, _, _ = self._p2_resolve_curated_series(payload, 'quarterly', 'cash')
        if not cash_values:
            cash_values, _, _, _ = self._p2_resolve_curated_series(payload, 'annual', 'cash')
        cash_and_bonds = float(cash_values[-1]) if cash_values else None
        if cash_and_bonds is None:
            cash_and_bonds = sg('totalCash')
        total_debt = self._p2_latest_total_debt_value(payload, 'quarterly')
        if total_debt is None:
            total_debt = self._p2_latest_total_debt_value(payload, 'annual')
        if total_debt is None:
    ...
    def _p2_apply_headline_metrics(self, values: dict[str, Any], targets: dict[str, Any]) -> None:
        """Write formatted headline metrics into one metrics row."""
        ...
        net_cash = values.get('net_cash')
        mktcap = values.get('mktcap')
        targets['pe'].setText(fmt_ratio(values.get('pe')))
        targets['fpe'].setText(fmt_ratio(values.get('fpe')))
        targets['ps'].setText(fmt_ratio(values.get('ps')))
        targets['peg'].setText(fmt_ratio(values.get('peg')))
        color_lbl(targets['fcf_margin'], f'{fcf_margin:.1f}%' if fcf_margin is not None else 'N/A')
        targets['ev_rev'].setText(fmt_ratio(values.get('ev_rev')))
        targets['ev_ebitda'].setText(fmt_ratio(values.get('ev_ebitda')))
        color_lbl(targets['net_cash'], fmt_num(net_cash) if net_cash is not None else 'N/A')
        targets['beta'].setText(fmt_ratio(values.get('beta'), suffix=''))
        targets['mktcap'].setText(fmt_num(mktcap) if mktcap is not None else 'N/A')
    ...
    def update_page2(self, data: Any, *, update_collection_info: bool=True, status_text: str | None=None) -> Any:
        ...
        self._p2_apply_headline_metrics(self._p2_compute_headline_metrics(data), self.p2_metric_vals)
        self._p2_set_metrics_ticker(self.p2_metric_ticker_lbl, ticker, 'primary')

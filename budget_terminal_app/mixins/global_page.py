...
class GlobalPageMixin:
    ...
    def init_page26(self) -> None:
        ...
        self.p26_export_2x_btn = QPushButton("Export 2x Upscale")
        self.p26_export_2x_btn.setToolTip("Copy the Global market map to the clipboard at 2x resolution.")
        self.set_theme_variant(self.p26_export_2x_btn, "positive")
        self.p26_export_2x_btn.clicked.connect(self._p26_export_map_2x)
        header.addWidget(self.p26_export_2x_btn)
        header.addStretch()
    ...
    def _p26_render_map_image(self, scale: Any = P26_EXPORT_SCALE) -> QImage:
        """Render the current Global map at a larger physical pixel size."""
        map_widget = getattr(self, "p26_map", None)
        ...
        try:
            ...
            return QImage()
        width = int(map_widget.width())
        height = int(map_widget.height())
        if not math.isfinite(scale_value) or scale_value <= 0 or width <= 0 or height <= 0:
            return QImage()
        pixel_width = int(round(width * scale_value))
        pixel_height = int(round(height * scale_value))
        ...
        if image.isNull():
            return image
        image.fill(QColor(self.theme_color("background_primary")))
    ...
    def _p26_export_map_2x(self) -> None:
        """Copy a true 2x render of the Global map to the clipboard."""
        if not self._p26_rows():
            self._p26_set_status("Load global index data before exporting.", "warning")
            return
        try:
            image = self._p26_render_map_image(P26_EXPORT_SCALE)
            if image.isNull():
                self._p26_set_status("Global map export failed: map is not ready.", "negative")
                return
            QApplication.clipboard().setImage(image)
        except Exception as exc:
            self._p26_set_status(f"Global map export failed: {exc}", "negative")
            QMessageBox.critical(self, "Export Failed", f"Unable to copy the Global map to the clipboard.\n\n{exc}")
            return
        self._p26_set_status(
            f"Global map copied to clipboard at 2x resolution ({image.width()}x{image.height()}).",
            "positive",
        )

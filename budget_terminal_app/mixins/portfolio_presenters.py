...
def format_market_cap(value: Any) -> str:
    ...
    if number is None:
        return "--"
    if isinstance(value, dict) and str(value.get("size_type") or "").lower() == "aum":
        return f"AUM ${format_market_cap_value(number)}"
...
def analyst_target_cell_text_and_sort(current_price: Any, analyst_target: Any) -> tuple[str, float]:
    if isinstance(analyst_target, dict):
        if analyst_target.get("unavailable_reason") == "not_applicable_for_etf":
            return "N/A", P4_MISSING_NUMERIC_SORT_VALUE
        analyst_target = analyst_target.get("target")

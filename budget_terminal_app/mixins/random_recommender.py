...
P18_FULL_PAYLOAD_LIMIT = 5
P18_WIDE_BREAKPOINT = 1400
P18_COMPACT_BREAKPOINT = 900
P18_CANDIDATE_FIELDS = (
    'symbol',
    'rank',
    'score',
    'pattern_type',
...
class RandomRecommenderMixin:
    ...
    def init_page18(self) -> None:
        ...
        self._p18_responsive_mode = None
        self._p18_splitter_sizes = {}
        ...
        self.p18_status_label.setMinimumWidth(140)
        self.p18_status_label.setMaximumWidth(420)
        self.p18_status_label.setWordWrap(True)
        self.set_theme_role(self.p18_status_label, 'status_muted')
        title_row.addWidget(self.p18_status_label)
        top_layout.addLayout(title_row)

        self.p18_header_controls_grid = QGridLayout()
        self.p18_header_controls_grid.setContentsMargins(0, 0, 0, 0)
        self.p18_header_controls_grid.setHorizontalSpacing(8)
        self.p18_header_controls_grid.setVerticalSpacing(5)
        ...
        self.p18_pattern_host = QWidget()
        self.p18_pattern_grid = QGridLayout(self.p18_pattern_host)
        self.p18_pattern_grid.setContentsMargins(0, 0, 0, 0)
        self.p18_pattern_grid.setHorizontalSpacing(10)
        self.p18_pattern_grid.setVerticalSpacing(4)
        ...
        self.p18_body_scroll = QScrollArea()
        self.p18_body_scroll.setFrameShape(QFrame.Shape.NoFrame)
        self.p18_body_scroll.setWidgetResizable(True)
        self.p18_body_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        self.p18_body_scroll.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded)
        self.p18_body_host = QWidget()
        self.p18_body_host.setMinimumWidth(0)
        body_host_layout = QVBoxLayout(self.p18_body_host)
        body_host_layout.setContentsMargins(0, 0, 0, 0)
        body_host_layout.setSpacing(0)
        self.p18_body_splitter = QSplitter(Qt.Orientation.Horizontal)
        self.p18_body_splitter.setChildrenCollapsible(False)
        body_host_layout.addWidget(self.p18_body_splitter)
        self.p18_body_scroll.setWidget(self.p18_body_host)
        self._p18_resize_timer = QTimer(self)
        self._p18_resize_timer.setSingleShot(True)
        self._p18_resize_timer.setInterval(50)
        self._p18_resize_timer.timeout.connect(self._p18_apply_responsive_layout)
        self.p18_body_scroll.viewport().installEventFilter(self)
        layout.addWidget(self.p18_body_scroll, 1)
        ...
        self.p18_body_splitter.addWidget(left_panel)
        self._p18_panel_widgets.extend([left_panel, why_box, action_box])

        self.p18_research_splitter = QSplitter(Qt.Orientation.Horizontal)
        self.p18_research_splitter.setChildrenCollapsible(False)
        self.p18_left_research_splitter = QSplitter(Qt.Orientation.Vertical)
        self.p18_left_research_splitter.setChildrenCollapsible(False)
        self.p18_right_research_splitter = QSplitter(Qt.Orientation.Vertical)
        self.p18_right_research_splitter.setChildrenCollapsible(False)
        ...
        chart_layout.addWidget(self.p18_chart_status)
        ...
        self.p18_left_research_splitter.addWidget(chart_frame)
        self.p18_left_research_splitter.addWidget(candidates_frame)
        self.p18_left_research_splitter.setStretchFactor(0, 2)
        self.p18_left_research_splitter.setStretchFactor(1, 3)

        self.p18_right_research_splitter.addWidget(options_frame)
        self.p18_right_research_splitter.addWidget(news_frame)
        self.p18_right_research_splitter.addWidget(history_frame)
        self.p18_right_research_splitter.setStretchFactor(0, 2)
        self.p18_right_research_splitter.setStretchFactor(1, 3)
        self.p18_right_research_splitter.setStretchFactor(2, 2)

        self.p18_research_splitter.addWidget(self.p18_left_research_splitter)
        self.p18_research_splitter.addWidget(self.p18_right_research_splitter)
        self.p18_research_splitter.setStretchFactor(0, 3)
        self.p18_research_splitter.setStretchFactor(1, 2)

        self.p18_body_splitter.addWidget(self.p18_research_splitter)
        self.p18_body_splitter.setStretchFactor(0, 2)
        self.p18_body_splitter.setStretchFactor(1, 5)
        for widget in (
            top_frame,
            self.p18_pattern_host,
            self.p18_body_scroll,
            self.p18_body_host,
            self.p18_body_splitter,
            left_panel,
            why_box,
            action_box,
            self.p18_research_splitter,
            self.p18_left_research_splitter,
            self.p18_right_research_splitter,
            chart_frame,
            options_frame,
            candidates_frame,
            news_frame,
            history_frame,
            self.p18_chart_plot,
            self.p18_chart_rsi_plot,
            self.p18_chart_macd_plot,
            self.p18_top_options_table,
            self.p18_candidates_table,
            self.p18_news_table,
            self.p18_history_table,
        ):
            self._p18_allow_horizontal_shrink(widget)
        self._p18_panel_widgets.extend([chart_frame, options_frame, candidates_frame, news_frame, history_frame])
        self._p18_apply_responsive_layout(force=True)
        QTimer.singleShot(0, lambda: self._p18_apply_responsive_layout(force=True))
        app = QApplication.instance()
        if app is not None:
            app.aboutToQuit.connect(self._p18_shutdown_workers)
        self._apply_random_recommender_theme()

    def _p18_allow_horizontal_shrink(self, widget: Any) -> None:
        if widget is None:
            return
    ...
    def eventFilter(self, watched: Any, event: Any) -> bool:
        """Keep Roll relayout local to its scroll viewport."""
        viewport = self.p18_body_scroll.viewport() if hasattr(self, 'p18_body_scroll') else None
        if watched is viewport and event.type() == QEvent.Type.Resize:
            timer = getattr(self, '_p18_resize_timer', None)
            if timer is not None:
                timer.start()
        return super().eventFilter(watched, event)

    def _p18_view_width(self) -> int:
        scroll = getattr(self, 'p18_body_scroll', None)
        if scroll is not None:
            try:
                ...
                pass
        try:
            return max(int(self.page18.width()), 0)
        except Exception:
            return 0

    def _p18_capture_splitter_sizes(self, mode: str) -> None:
        if not mode:
            return
        sizes = {}
        for key, splitter in (
            ('body', getattr(self, 'p18_body_splitter', None)),
            ('research', getattr(self, 'p18_research_splitter', None)),
            ('left', getattr(self, 'p18_left_research_splitter', None)),
            ('right', getattr(self, 'p18_right_research_splitter', None)),
        ):
            if splitter is not None:
                current = [int(value) for value in splitter.sizes()]
                if current and sum(current) > 0:
                    sizes[key] = current
        if sizes:
            self._p18_splitter_sizes[mode] = sizes

    def _p18_restore_splitter_sizes(self, mode: str) -> None:
        saved = dict(getattr(self, '_p18_splitter_sizes', {}).get(mode, {}) or {})
        defaults = {
            'wide': {'body': [2, 5], 'research': [3, 2], 'left': [2, 3], 'right': [2, 3, 2]},
            'medium': {'body': [2, 5], 'research': [3, 2], 'left': [2, 3], 'right': [2, 3, 2]},
            'compact': {'body': [2, 7], 'research': [5, 6], 'left': [2, 3], 'right': [2, 3, 2]},
        }.get(mode, {})
        for key, splitter in (
            ('body', getattr(self, 'p18_body_splitter', None)),
            ('research', getattr(self, 'p18_research_splitter', None)),
            ...
            if splitter is not None:
                splitter.setSizes(saved.get(key) or defaults.get(key) or [])

    def _p18_reflow_pattern_grid(self, columns: int, *, compact_header: bool) -> None:
        for checkbox in getattr(self, '_p18_pattern_checkboxes', []):
            self.p18_pattern_grid.removeWidget(checkbox)
        for column in range(6):
            self.p18_pattern_grid.setColumnStretch(column, 0)
        for index, checkbox in enumerate(getattr(self, '_p18_pattern_checkboxes', [])):
            self.p18_pattern_grid.addWidget(checkbox, index // columns, index % columns)
        for column in range(columns):
            self.p18_pattern_grid.setColumnStretch(column, 1)

        self.p18_header_controls_grid.removeWidget(self.p18_roll_btn)
        self.p18_header_controls_grid.removeWidget(self.p18_pattern_host)
        if compact_header:
            self.p18_header_controls_grid.addWidget(self.p18_roll_btn, 0, 0)
            self.p18_header_controls_grid.addWidget(self.p18_pattern_host, 1, 0, 1, 2)
        else:
            self.p18_header_controls_grid.addWidget(self.p18_roll_btn, 0, 0)
            self.p18_header_controls_grid.addWidget(self.p18_pattern_host, 0, 1)
        self.p18_header_controls_grid.setColumnStretch(0, 0)
        self.p18_header_controls_grid.setColumnStretch(1, 1)

    def _p18_apply_responsive_layout(self, *, force: bool=False) -> None:
        if not hasattr(self, 'p18_body_splitter'):
            return
        width = self._p18_view_width()
        if width >= P18_WIDE_BREAKPOINT:
            mode = 'wide'
            pattern_columns = 6
        elif width >= P18_COMPACT_BREAKPOINT:
            mode = 'medium'
            pattern_columns = 3
        else:
            mode = 'compact'
            pattern_columns = 2
        previous_mode = getattr(self, '_p18_responsive_mode', None)
        if not force and previous_mode == mode:
            return
        if previous_mode:
            self._p18_capture_splitter_sizes(previous_mode)

        is_wide = mode == 'wide'
        is_compact = mode == 'compact'
        self.p18_body_splitter.setOrientation(
            Qt.Orientation.Horizontal if is_wide else Qt.Orientation.Vertical
        )
        self.p18_research_splitter.setOrientation(
            Qt.Orientation.Vertical if is_compact else Qt.Orientation.Horizontal
        )
        self._p18_reflow_pattern_grid(pattern_columns, compact_header=is_compact)
        for column in (5, 7, 8):
            self.p18_candidates_table.setColumnHidden(column, not is_wide)

        minimum_heights = {'wide': 620, 'medium': 960, 'compact': 1720}
        self.p18_body_host.setMinimumHeight(minimum_heights[mode])
        self.p18_body_host.setMinimumWidth(0)
        self._p18_responsive_mode = mode
        self._p18_restore_splitter_sizes(mode)
    ...
    def _p18_set_status(self, text: Any, status: str='muted', *, include_global: bool=True) -> None:
        ...
        self.p18_status_label.setToolTip(str(text or ''))
    ...
    def _p18_set_busy(self, busy: bool, *, status_text: Any=None, include_global: bool=True) -> None:
        ...
        if status_text not in (None, ''):
            self._p18_set_status(status_text, 'info' if busy else 'muted', include_global=include_global)

    def _p18_roll_stock(self, *_: Any, include_global_status: bool=True, target_symbol: Any='') -> None:
        active_worker = getattr(self, 'p18_worker', None)
        if active_worker is not None and hasattr(active_worker, 'cancel'):
            try:
        ...
        self._p18_set_busy(True, status_text=loading_text, include_global=include_global_status)
        ...
        thread = QThread()
        self._p18_inflight_workers[request_id] = (worker, thread)
    ...
    def _p18_shutdown_workers(self) -> None:
        """Cancel and briefly drain all Roll workers before application shutdown."""
        inflight = list(getattr(self, '_p18_inflight_workers', {}).values())
        for worker, thread in inflight:
            try:
                worker.cancel()
            except Exception:
                pass
            try:
                ...
                pass
        for _worker, thread in inflight:
            try:
                if thread.isRunning():
                    thread.wait(400)
            except Exception:
    ...
    def _p18_handle_progress(self, request_id: int, update: Any) -> None:
        if request_id != getattr(self, '_p18_active_request', 0):
            return
        progress = dict(update) if isinstance(update, dict) else {}
        embedded_request = progress.get('request_id')
        try:
            if embedded_request is not None and int(embedded_request) != request_id:
                return
        except (TypeError, ValueError):
            return
        stage = str(progress.get('stage') or '').strip().casefold()
        message = str(progress.get('message') or '').strip()
        labels = {
            'screening': 'Screening liquid US equities',
            'history': 'Fetching one-year daily history',
            'candidates': 'Ranking pattern candidates',
            'core': 'Loading selected company',
            'enrichment': 'Loading chart, news, and options',
        }
        if not message:
            message = labels.get(stage, 'Rolling')
        current = progress.get('current')
        total = progress.get('total')
        try:
            if int(total) > 0 and int(current) >= 0:
    ...
    def _p18_handle_partial(self, request_id: int, update: Any) -> None:
        if request_id != getattr(self, '_p18_active_request', 0) or not isinstance(update, dict):
            return
        embedded_request = update.get('request_id')
        try:
            if embedded_request is not None and int(embedded_request) != request_id:
                return
        except (TypeError, ValueError):
            return
        section = str(update.get('section') or '').strip().casefold()
        patch = update.get('payload')
        ...
        if section == 'candidates':
            ...
            self._p18_render_candidates(candidates, pattern_modes=patch.get('pattern_modes'))
            return
        if section in ('core', 'metadata'):
            self._p18_payload_request_id = request_id
            self._p18_apply_payload(pending, save_snapshot=False, render_optional=False)
            if section == 'core':
                self._p18_render_chart({})
                self.p18_chart_status.setText('Loading one-year daily history…')
                self._p18_render_news([])
                self.p18_news_empty.setText('Loading recent headlines…')
                self._p18_render_top_options([], 'Loading options expirations…')
            return

        self._p18_loaded_payload = pending
        self._p18_payload_request_id = request_id
        if section == 'chart':
            self._p18_render_chart(patch.get('chart_history') or {})
        elif section == 'news':
            self._p18_render_news(
                patch.get('news') or [],
                str(patch.get('news_status') or ''),
            )
        elif section == 'options':
            self._p18_render_top_options(
                patch.get('top_options') or [],
                str(patch.get('top_options_status') or ''),
            )
    ...
    def _p18_result_status(self, payload: dict[str, Any]) -> tuple[str, str]:
        ...
        warning_text = self._p18_warning_text(payload.get('warnings'))
        match_tier = str(payload.get('match_tier') or '').strip().casefold()
    ...
    def _p18_compact_candidate(self, candidate: Any) -> dict[str, Any]:
        if not isinstance(candidate, dict):
            return {}
        return {key: candidate.get(key) for key in P18_CANDIDATE_FIELDS if key in candidate}

    def _p18_compact_payload(self, payload: Any) -> dict[str, Any]:
        if not isinstance(payload, dict):
            return {}
        compact = {key: payload.get(key) for key in P18_PAYLOAD_FIELDS if key in payload}
        info = payload.get('info') if isinstance(payload.get('info'), dict) else {}
        quote = payload.get('quote') if isinstance(payload.get('quote'), dict) else {}
        compact['info'] = {key: info.get(key) for key in P18_INFO_FIELDS if key in info}
        compact['quote'] = {key: quote.get(key) for key in P18_QUOTE_FIELDS if key in quote}
        compact['candidate_pool'] = [
            candidate
            for candidate in (self._p18_compact_candidate(item) for item in list(payload.get('candidate_pool') or []))
            if candidate
        ]
        history = payload.get('chart_history') if isinstance(payload.get('chart_history'), dict) else {}
        compact['chart_history'] = {
            key: list(history.get(key) or [])
        ...
        compact['top_options'] = [
            {
                key: item.get(key)
                for key in ('ticker', 'type', 'expiration', 'strike', 'lastPrice', 'volume', 'openInterest', 'impliedVolatility')
                if key in item
            }
            for item in list(payload.get('top_options') or [])
            if isinstance(item, dict)
        ]
        compact['news'] = [
            {
                key: item.get(key)
                for key in ('ticker', 'title', 'source', 'time', 'url', 'category', '_ts')
                if key in item
            }
            for item in list(payload.get('news') or [])
            if isinstance(item, dict)
        ]
        return compact

    def _p18_compact_history_item(self, item: Any) -> dict[str, Any]:
        if not isinstance(item, dict):
            return {}
        return {
            key: item.get(key)
            for key in ('symbol', 'company', 'sector', 'rolled_at')
            if item.get(key) not in (None, '')
        }

    def _p18_remember_full_payload(self, payload: Any) -> None:
        if not isinstance(payload, dict):
            return
        symbol = str(payload.get('symbol') or '').upper().strip()
        if not symbol:
            return
        cache = getattr(self, '_p18_full_payloads', None)
        if not isinstance(cache, dict):
            cache = {}
        cache.pop(symbol, None)
        cache[symbol] = payload
        while len(cache) > P18_FULL_PAYLOAD_LIMIT:
            cache.pop(next(iter(cache)), None)
        self._p18_full_payloads = cache

    def _p18_set_pattern_modes(self, modes: Any) -> None:
        selected = {str(mode or '').strip().casefold() for mode in list(modes or [])}
        mapping = {
            'breakout': getattr(self, 'p18_breakout_checkbox', None),
            'consolidation': getattr(self, 'p18_consolidation_checkbox', None),
            'downtrend': getattr(self, 'p18_downtrend_checkbox', None),
            'double_bottom': getattr(self, 'p18_double_bottom_checkbox', None),
            'bullish_flag': getattr(self, 'p18_bullish_flag_checkbox', None),
            'bullish_rsi_divergence': getattr(self, 'p18_bullish_rsi_divergence_checkbox', None),
        }
        for mode, checkbox in mapping.items():
            if checkbox is not None:
                checkbox.setChecked(mode in selected)
    ...
    def _p18_session_snapshot(self) -> dict[str, Any] | None:
        ...
        return {
            'symbol': symbol,
            'payload': serialize_session_value(self._p18_compact_payload(payload)),
            'pattern_modes': list(self._p18_selected_pattern_modes()),
            'history': serialize_session_value([
                row
                for row in (
                    self._p18_compact_history_item(item)
                    for item in list(getattr(self, '_p18_roll_history', []))[:P18_HISTORY_LIMIT]
                )
                if row
            ]),
        }
    ...
    def _p18_restore_session_snapshot(self, snapshot: Any) -> bool:
        ...
        self._p18_roll_history = [
            row
            for row in (self._p18_compact_history_item(item) for item in list(history or []))
            if row
        ][:P18_HISTORY_LIMIT]
        pattern_modes = payload.get('pattern_modes', restored_payload.get('pattern_modes'))
        self._p18_set_pattern_modes(pattern_modes)
        self._p18_apply_payload(restored_payload, save_snapshot=False)
        self._p18_remember_full_payload(restored_payload)
    ...
    def _p18_apply_payload(
        ...
        *,
        ...
        self._p18_loaded_payload = dict(payload or {})
        payload = self._p18_loaded_payload
        symbol = str(payload.get('symbol', '') or '').upper().strip()
        info = payload.get('info', {}) if isinstance(payload.get('info', {}), dict) else {}
        quote = payload.get('quote', {}) if isinstance(payload.get('quote', {}), dict) else {}
        ...
        if 'candidate_pool' in payload:
            self._p18_render_candidates(payload.get('candidate_pool') or [], pattern_modes=payload.get('pattern_modes'))
        if render_optional:
            ...
            self._p18_render_news(payload.get('news') or [], payload.get('news_status') or '')
        self._p18_update_action_buttons()
        if save_snapshot:
            self._p18_remember_full_payload(payload)
            self._p18_record_history(payload)
            self._p18_save_session_snapshot()

    def _p18_render_candidates(self, candidates: Any, *, pattern_modes: Any=None) -> None:
        ...
        if not clean_candidates:
            ...
            return
        active_modes = list(pattern_modes or [])
        if not active_modes:
            active_modes = self._p18_payload_pattern_modes(getattr(self, '_p18_loaded_payload', None))
        pattern_active = bool(active_modes)
        current_symbol = self._p18_current_symbol()
        self.p18_candidates_table.setUpdatesEnabled(False)
        try:
            ...
            for row_index, candidate in enumerate(clean_candidates):
                ...
                show_pattern_score = (
                    match_tier in ('strict', 'near', 'fallback')
                    and isinstance(pattern_score, (int, float))
                    and math.isfinite(float(pattern_score))
                    and float(pattern_score) > 0
                )
                ...
                values = (
                    ...
                    setup_text,
                    self._p18_format_score(pattern_score) if show_pattern_score else 'N/A',
            ...
            self.p18_candidates_table.setUpdatesEnabled(True)
    ...
    def _p18_update_why_panel(self, payload: dict[str, Any], info: dict[str, Any], quote: dict[str, Any]) -> None:
        ...
        if pattern_type and pattern_type != 'None':
            ...
            reason_text = f'{pattern_type} setup {pattern_score_text}: ' + (
                ', '.join(pattern_reasons[:3]) if pattern_reasons else 'technical setup scored'
            )
            technical_text = self._p18_technical_snapshot_text(payload)
            if technical_text:
                reason_text = f'{reason_text}; {technical_text}'
        elif pattern_modes:
            reason_text = fallback_reason or (
                'pattern history unavailable; showing balanced scored candidates'
    ...
    def _p18_update_why_metrics(
        ...
        primary_mode = str(payload.get('primary_pattern_mode') or '').strip() if isinstance(payload, dict) else ''
    ...
    def _p18_technical_snapshot_text(self, payload: dict[str, Any]) -> str:
        ...
        if not isinstance(snapshot, dict):
            return ''
        primary_mode = str(payload.get('primary_pattern_mode') or '').strip()
        if primary_mode and isinstance(snapshot.get(primary_mode), dict):
            snapshot = {primary_mode: snapshot[primary_mode]}
    ...
    def _p18_render_top_options(self, records: Any, status_text: str='') -> None:
        ...
        if not clean_records:
            ...
            return
        self.p18_top_options_table.setUpdatesEnabled(False)
        try:
            ...
            self.p18_top_options_table.setUpdatesEnabled(True)
        self.p18_top_options_empty.setText(status_text)
        self.p18_top_options_empty.setVisible(bool(status_text))
    ...
    def _p18_render_history(self) -> None:
        ...
        if not history:
            ...
            return
        self.p18_history_table.setUpdatesEnabled(False)
        try:
            ...
            for row_index, item in enumerate(history):
                ...
                for col_index, value in enumerate(values):
                    ...
                    if col_index == 0:
                        table_item.setData(Qt.ItemDataRole.UserRole, symbol)
                        table_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                    elif col_index == 3:
                        table_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                    self.p18_history_table.setItem(row_index, col_index, table_item)
        finally:
            self.p18_history_table.setUpdatesEnabled(True)
    ...
    def _p18_open_history_item(self, item: Any) -> None:
        ...
        stored = ticker_item.data(Qt.ItemDataRole.UserRole) if ticker_item is not None else None
        symbol = ''
        payload = None
        if isinstance(stored, dict):
            ...
            symbol = str(stored or (ticker_item.text() if ticker_item is not None else '')).upper().strip()
            payload = getattr(self, '_p18_full_payloads', {}).get(symbol)
        if isinstance(payload, dict):
            self._p18_apply_payload(payload, save_snapshot=False)
            self._p18_set_status(f'Revisited {symbol} from roll history.', 'positive')
            self._p18_save_session_snapshot()
        elif symbol:
            self._p18_roll_stock(target_symbol=symbol)

    def _p18_render_news(self, articles: Any, status_text: str='') -> None:
        ...
        if not clean_articles:
            ...
            self.p18_news_empty.setText(
                str(status_text or '').strip()
                or 'No recent headlines were available for this ticker.'
            )
            return
        self.p18_news_table.setUpdatesEnabled(False)
        try:
            self._populate_news_table(self.p18_news_table, clean_articles)
        finally:
            self.p18_news_table.setUpdatesEnabled(True)

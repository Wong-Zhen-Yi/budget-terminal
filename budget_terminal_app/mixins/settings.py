...
class SettingsMixin:
    ...
    def _build_settings_user_data_box(self) -> QGroupBox:
        ...
        action_grid.addWidget(export_btn, 0, 0, 1, 2)
    ...
    def _apply_runtime_user_data(self, payload: Any) -> None:
        ...
        page28_initialized = self._page_initialized(page_attr='page28')
        ...
        self.charts_options_top_volume_page_state = save_charts_options_top_volume_page_settings(
            payload.get('charts_options_top_volume_page', DEFAULT_CHARTS_OPTIONS_TOP_VOLUME_PAGE_SETTINGS)
        ) if isinstance(payload, dict) else save_charts_options_top_volume_page_settings(DEFAULT_CHARTS_OPTIONS_TOP_VOLUME_PAGE_SETTINGS)
        charts_options_top_volume_page_state = dict(self.charts_options_top_volume_page_state)
        ...
        self.up_down_page_state = save_up_down_page_settings(payload.get('up_down_page', DEFAULT_UP_DOWN_PAGE_SETTINGS)) if isinstance(payload, dict) else save_up_down_page_settings(DEFAULT_UP_DOWN_PAGE_SETTINGS)
        up_down_page_state = dict(self.up_down_page_state)
        ...
        if hasattr(self, 'p27_tabs'):
            self.p27_active_source = str(up_down_page_state.get('active_source', 'portfolio') or 'portfolio').strip().lower()
            self.p27_interval_key = str(up_down_page_state.get('interval_key', '1d') or '1d').strip().lower()
            self.p27_custom_symbols = list(up_down_page_state.get('custom_symbols', []))
        ...
        if page28_initialized and hasattr(self, 'p28_symbol_input'):
            self.p28_symbol = str(charts_options_top_volume_page_state.get('symbol', 'SPY') or 'SPY').upper().strip()
            self.p28_timeframe_label = str(charts_options_top_volume_page_state.get('timeframe_label', '1 Day') or '1 Day').strip()
            self.p28_type_filter = self._p28_normalize_type_filter(charts_options_top_volume_page_state.get('type_filter', 'calls'))
            self.p28_expiration_scope = 'all'
            self.p28_splitter_sizes = list(charts_options_top_volume_page_state.get('splitter_sizes', [5, 3]))
            self.p28_symbol_input.setText(self.p28_symbol)
            self.p28_symbol_label.setText(self.p28_symbol)
            self._p28_payload = self._p28_empty_payload(self.p28_symbol)
            self._p28_has_completed_view = False
            self._p28_render_chart(pd.DataFrame(), '1d')
            ...
            self._p28_update_expiration_scope_buttons()
            self._p28_apply_splitter_sizes()
            self._p28_set_status('Projections settings restored.', 'positive')
        if hasattr(self, 'dashboard_symbol_input'):
    ...
    def _settings_choose_backup_directory(self, title: str) -> str | None:
        """Prompt for an existing folder used as an export parent or import source."""
    ...
    def _settings_backup_bundle_summary(self, bundle: Any) -> str:
        """Build the combined folder preview shown before import."""
        user_data = bundle.payloads[USER_DATA_KIND]
        portfolio_order = user_data.get('portfolio_order', [])
        ...
        cards = list(bundle.payloads[CARDS_KIND].get('custom_cards', []))
        equal_count = sum(1 for card in cards if card.get('weighting') == 'equal')
        custom_count = len(cards) - equal_count
        paper_tables = bundle.payloads[PAPER_TRADING_KIND].get('tables', {})
        account_count = len(paper_tables.get('accounts', []))
        order_count = len(paper_tables.get('orders', []))
        ignored_names = [path.name for path in bundle.unrecognized_files]
        if len(ignored_names) > 6:
            ignored_text = f'{", ".join(ignored_names[:6])}, +{len(ignored_names) - 6} more'
        else:
            ignored_text = ', '.join(ignored_names) or 'None'

        return '\n'.join([
            f'Folder: {bundle.folder}',
            '',
            f'User data: {bundle.paths[USER_DATA_KIND].name}',
            f'  Exported at: {bundle.metadata[USER_DATA_KIND].get("exported_at") or "-"}',
            f'  App version: {bundle.metadata[USER_DATA_KIND].get("app_version") or "-"}',
    ...
    def _settings_confirm_backup_bundle_import(self, bundle: Any) -> bool:
        ...
        import_btn = prompt.addButton('Import Backup Folder', QMessageBox.ButtonRole.AcceptRole)
    ...
    def _on_export_backup_bundle(self) -> None:
        ...
        try:
            ...
            if self._page_initialized(page_attr='page29') and hasattr(self, '_p29_save_state'):
                self._p29_save_state()
            bundle = export_backup_bundle(parent)
        except Exception as exc:
            self._set_settings_status(f'Backup-folder export failed: {exc}', 'negative')
    ...
    def _settings_refresh_after_backup_import(
        ...
        if self._page_initialized(page_attr='page29'):
            self.strategies_state = result.cards['state']
            if hasattr(self, '_p29_performance_cache'):
    ...
    def _on_import_backup_bundle(self) -> None:
        ...
        if not folder:
            ...
            return
        try:
            bundle = discover_backup_bundle(folder)
        except BackupBundleError as exc:
            self._set_settings_status(f'Backup-folder import failed: {exc}', 'negative')
            QMessageBox.critical(
                self,
                'Import Failed',
                f'The selected folder is not a complete valid backup.\n\n{exc}',
            )
            return
        if not self._settings_confirm_backup_bundle_import(bundle):
            self._set_settings_status('Backup-folder import cancelled.')
            return
        ...
        try:
            ...
            if self._page_initialized(page_attr='page29') and hasattr(self, '_p29_save_state'):
                self._p29_save_state()
            self._settings_cancel_pending_runtime_updates()
            result = apply_backup_bundle(bundle)
        except Exception as exc:
            self._set_settings_status(f'Backup-folder import failed: {exc}', 'negative')
            QMessageBox.critical(
                self,
                'Import Failed',
                f'Unable to import the complete backup folder.\n\n{exc}',
            )
            return

        try:
            self._settings_refresh_after_backup_import(result)
        except Exception as exc:
            self._set_settings_status('Backup imported, but the live UI refresh failed.', 'negative')
            QMessageBox.critical(
                self,
                'Refresh Failed',
                'The backup folder was imported, but the live UI could not be fully refreshed. '
                f'Restart Budget Terminal to load the imported data.\n\n{exc}',
            )
            return

        added = int(result.cards.get('added_count', 0) or 0)
        updated = int(result.cards.get('updated_count', 0) or 0)
        ignored = len(bundle.unrecognized_files)
        self._set_settings_status(f'Backup folder imported from {bundle.folder}', 'positive')
        QMessageBox.information(
            self,
            'Import Complete',
    ...
    def _settings_default_strategies_path(self) -> str:
        """Return the default JSON path used to export custom strategy cards."""
    ...
    def _on_export_custom_strategies(self) -> None:
        """Export user-created strategy cards into a portable JSON file."""
        path, _ = QFileDialog.getSaveFileName(
            self,
            'Export Custom Cards',
            self._settings_default_strategies_path(),
            'JSON files (*.json)',
        )
        export_path = str(path or '').strip()
        if not export_path:
            self._set_settings_status('Custom card export cancelled.')
            return
        if not export_path.lower().endswith('.json'):
            export_path = f'{export_path}.json'
        try:
            ...
            self._set_settings_status(f'Custom card export failed: {exc}', 'negative')
            QMessageBox.critical(self, 'Export Failed', f'Unable to export custom cards.\n\n{exc}')
            return
        self._set_settings_status(f'Custom cards exported to {export_path}', 'positive')
        QMessageBox.information(
            self,
            'Export Complete',
            f'Custom cards exported successfully.\n\nFile: {export_path}',
        )

    def _on_import_custom_strategies(self) -> None:
        """Validate, preview, and merge custom cards from a portable JSON export."""
        path, _ = QFileDialog.getOpenFileName(
            self,
            'Import Custom Cards',
            self._settings_default_strategies_path(),
        ...
        if not import_path:
            self._set_settings_status('Custom card import cancelled.')
            return
        try:
            payload = load_custom_strategies_import(import_path)
        except Exception as exc:
            self._set_settings_status(f'Custom card import failed: {exc}', 'negative')
            QMessageBox.critical(self, 'Import Failed', f'Unable to read custom cards.\n\n{exc}')
            return

        cards = list(payload.get('custom_cards', []))
        equal_count = sum(1 for card in cards if card.get('weighting') == 'equal')
        custom_count = len(cards) - equal_count
        names = ', '.join(str(card.get('name', 'Untitled Card')) for card in cards[:6])
        if len(cards) > 6:
            names = f'{names}, +{len(cards) - 6} more'
        prompt = QMessageBox(self)
        prompt.setWindowTitle('Import Custom Cards')
        prompt.setIcon(QMessageBox.Icon.Question)
        prompt.setText(f'Merge {len(cards)} custom card(s) into the current Cards page?')
        prompt.setInformativeText('\n'.join([
            f'Source: {import_path}',
            f'Exported at: {payload.get("exported_at") or "-"}',
            f'Equal weight: {equal_count} | Custom weights: {custom_count}',
            f'Cards: {names}',
            '',
            'Matching card IDs will be updated. New card IDs will be appended; existing unrelated cards are preserved.',
        ]))
        import_button = prompt.addButton('Import Custom Cards', QMessageBox.ButtonRole.AcceptRole)
        prompt.addButton(QMessageBox.StandardButton.Cancel)
        prompt.exec()
        ...
        try:
            result = merge_custom_strategies_import(payload)
            if self._page_initialized(page_attr='page29'):
                self.strategies_state = result['state']
                if hasattr(self, '_p29_performance_cache'):
                    self._p29_performance_cache.clear()
                if hasattr(self, '_p29_refresh_cards'):
                    self._p29_refresh_cards(request_data=False)
            ...
            self._set_settings_status(f'Custom card import failed: {exc}', 'negative')
            QMessageBox.critical(self, 'Import Failed', f'Unable to merge custom cards.\n\n{exc}')
            return
        ...
        summary = f'Imported custom cards: {added} added, {updated} updated'
        if skipped:
            summary = f'{summary}, {skipped} skipped'
        self._set_settings_status(summary, 'positive')
    ...
    def _on_clear_user_data(self) -> None:
        ...
        try:
            ...
            self._apply_runtime_user_data(normalized)
            cleared_cards_state = clear_custom_strategies()
            if self._page_initialized(page_attr='page29'):
                self.strategies_state = cleared_cards_state
                if hasattr(self, '_p29_performance_cache'):
                    self._p29_performance_cache.clear()
                if hasattr(self, '_p29_refresh_cards'):
                    self._p29_refresh_cards(request_data=False)
            ...
            QMessageBox.critical(self, 'Clear Failed', f'Unable to clear user data and cards.\n\n{exc}')
            return
        self._set_settings_status('All user data and custom cards cleared.', 'positive')
        QMessageBox.information(self, 'Clear Complete', 'All saved user data and custom cards have been cleared.')

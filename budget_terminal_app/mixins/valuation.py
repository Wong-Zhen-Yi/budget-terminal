...
class ValuationMixin:
    ...
    def init_page23(self) -> None:
        ...
        self.valuation_assumption_ticker = str(self.valuation_page_state.get('last_ticker', 'NVDA') or 'NVDA').upper().strip()
        ...
        self.valuation_company_label.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Preferred)
        self.valuation_company_label.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)
        ...
        header_top.addWidget(QLabel('Ticker'))
        header_top.addWidget(self.valuation_ticker_input)
        header_top.addWidget(self.valuation_load_btn)
        header_top.addSpacing(8)
        header_top.addWidget(self.valuation_company_label, 1)
        header_meta.addWidget(self.valuation_price_label)
        header_meta.addWidget(self.valuation_market_cap_label)
        header_meta.addWidget(self.valuation_sector_label, 1)
        header_meta.addWidget(self.valuation_refresh_label)
        ...
        self.valuation_growth_1_5_spin = self._valuation_double_spin(-30.0, 50.0, '', 1, suffix=' %')
        self.valuation_growth_6_10_spin = self._valuation_double_spin(-15.0, 30.0, '', 1, suffix=' %')
        self.valuation_discount_spin = self._valuation_double_spin(4.0, 30.0, '', 1, suffix=' %')
        self.valuation_terminal_growth_spin = self._valuation_double_spin(-2.0, 5.0, '', 1, suffix=' %')
        self.valuation_exit_multiple_spin = self._valuation_double_spin(3.0, 50.0, '', 1, suffix=' x')
        self.valuation_terminal_method_combo = QComboBox()
        self.valuation_terminal_method_combo.addItems(['Gordon Growth', 'Exit Multiple'])
        self.valuation_projection_years_spin = QSpinBox()
        self.valuation_projection_years_spin.setRange(5, 15)
        self.valuation_projection_years_spin.setSuffix(' years')
        self.valuation_margin_spin = self._valuation_double_spin(0.0, 50.0, '', 1, suffix=' %')
        self.valuation_basis_type_combo.currentIndexChanged.connect(self._valuation_on_basis_type_changed)
        self.valuation_terminal_method_combo.currentIndexChanged.connect(self._valuation_update_dynamic_assumption_rows)
        controls = (
            ...
            self.valuation_terminal_method_combo,
        )
        for control in controls:
            signal = getattr(control, 'valueChanged', None) or getattr(control, 'currentIndexChanged', None)
            if signal is not None:
                signal.connect(self._valuation_on_assumption_edited)
        self.valuation_starting_form = self._valuation_assumption_group('Starting Metric')
        self.valuation_forecast_form = self._valuation_assumption_group('Forecast')
        self.valuation_terminal_form = self._valuation_assumption_group('Risk / Terminal')
        self.valuation_basis_value_label = QLabel('Starting FCF/share')
        self.valuation_terminal_method_label = QLabel('Terminal method')
        self.valuation_terminal_growth_label = QLabel('Terminal growth')
        self.valuation_exit_multiple_label = QLabel('Exit multiple')
        self.valuation_starting_form.addRow('Basis type', self.valuation_basis_type_combo)
        self.valuation_starting_form.addRow(self.valuation_basis_value_label, self.valuation_basis_value_spin)
        self.valuation_forecast_form.addRow('Growth years 1-5', self.valuation_growth_1_5_spin)
        self.valuation_forecast_form.addRow('Growth years 6-N', self.valuation_growth_6_10_spin)
        self.valuation_forecast_form.addRow('Projection years', self.valuation_projection_years_spin)
        self.valuation_terminal_form.addRow('Required return', self.valuation_discount_spin)
        self.valuation_terminal_form.addRow(self.valuation_terminal_method_label, self.valuation_terminal_method_combo)
        ...
        self.valuation_validation_label.hide()
        model_layout.addWidget(self.valuation_validation_label)
        ...
        self.valuation_detail_tabs.addTab(self._valuation_searchable_notes_page(), 'Notes')
        self.valuation_detail_tabs.addTab(self._valuation_searchable_sources_page(), 'Sources')
        ...
        initial_ticker = self._valuation_current_ticker()
        initial_by_ticker = self.valuation_page_state.get('assumptions_by_ticker', {})
        initial_assumptions = initial_by_ticker.get(initial_ticker) if isinstance(initial_by_ticker, dict) else None
        self._valuation_apply_assumptions_to_controls(initial_assumptions or self.valuation_page_state.get('assumptions', DEFAULT_VALUATION_ASSUMPTIONS))
    ...
    def _valuation_assumption_group(self, title: str) -> Any:
        frame = QFrame()
        ...
        form._valuation_group_frame = frame
        return form

    def _valuation_searchable_notes_page(self) -> Any:
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(6, 6, 6, 6)
        layout.setSpacing(6)
        toolbar = QHBoxLayout()
        self.valuation_notes_search = QLineEdit()
        self.valuation_notes_search.setPlaceholderText('Find in notes')
        self.valuation_notes_search.returnPressed.connect(self._valuation_find_in_notes)
        find_button = QPushButton('Find next')
        find_button.clicked.connect(self._valuation_find_in_notes)
        toolbar.addWidget(self.valuation_notes_search, 1)
        toolbar.addWidget(find_button)
        layout.addLayout(toolbar)
        layout.addWidget(self.valuation_notes_edit, 1)
        return page

    def _valuation_searchable_sources_page(self) -> Any:
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(6, 6, 6, 6)
        layout.setSpacing(6)
        self.valuation_source_search = QLineEdit()
        self.valuation_source_search.setPlaceholderText('Filter sources')
        self.valuation_source_search.textChanged.connect(self._valuation_filter_sources)
        layout.addWidget(self.valuation_source_search)
        layout.addWidget(self.valuation_source_table, 1)
        return page

    def _valuation_find_in_notes(self) -> None:
        query = self.valuation_notes_search.text().strip()
        if not query:
            return
        if not self.valuation_notes_edit.find(query):
            cursor = self.valuation_notes_edit.textCursor()
            cursor.movePosition(cursor.MoveOperation.Start)
            self.valuation_notes_edit.setTextCursor(cursor)
            self.valuation_notes_edit.find(query)

    def _valuation_filter_sources(self, text: str) -> None:
        query = str(text or '').strip().casefold()
        for row in range(self.valuation_source_table.rowCount()):
            values = [
                self.valuation_source_table.item(row, column).text()
                for column in range(self.valuation_source_table.columnCount())
                if self.valuation_source_table.item(row, column) is not None
            ]
            self.valuation_source_table.setRowHidden(row, bool(query) and query not in ' '.join(values).casefold())
    ...
    def _valuation_settings_payload(self) -> dict[str, Any]:
        ticker = self._valuation_current_ticker() or 'NVDA'
        ...
        assumptions_by_ticker = dict(getattr(self, 'valuation_page_state', {}).get('assumptions_by_ticker', {}))
        assumption_ticker = str(getattr(self, 'valuation_assumption_ticker', '') or ticker).upper().strip()
        assumptions_by_ticker[assumption_ticker] = assumptions
        current_assumptions = assumptions_by_ticker.get(ticker, normalize_valuation_assumptions(DEFAULT_VALUATION_ASSUMPTIONS))
        return {
            'last_ticker': ticker,
    ...
    def _valuation_assumptions_from_controls(self) -> dict[str, Any]:
        return normalize_valuation_assumptions({
            ...
            'terminal_method': 'gordon_growth' if self.valuation_terminal_method_combo.currentText() == 'Gordon Growth' else 'exit_multiple',
    ...
    def _valuation_update_dynamic_assumption_rows(self, *_: Any) -> None:
        if not hasattr(self, 'valuation_terminal_method_combo'):
            return
        is_eps = self.valuation_basis_type_combo.currentText().upper() == 'EPS'
        is_gordon = not is_eps and self.valuation_terminal_method_combo.currentText() == 'Gordon Growth'
        self.valuation_basis_value_label.setText('Starting EPS' if is_eps else 'Starting FCF/share')
        self.valuation_terminal_method_label.setVisible(not is_eps)
        self.valuation_terminal_method_combo.setVisible(not is_eps)
        self.valuation_terminal_growth_label.setVisible(is_gordon)
        self.valuation_terminal_growth_spin.setVisible(is_gordon)
        self.valuation_exit_multiple_label.setText('Exit P/E' if is_eps else 'Exit multiple')
    ...
    def _valuation_apply_assumptions_to_controls(self, assumptions: Any) -> None:
        ...
        try:
            ...
            terminal_method = str(values.get('terminal_method', 'gordon_growth')).strip().casefold().replace(' ', '_')
            self.valuation_terminal_method_combo.setCurrentText('Exit Multiple' if terminal_method == 'exit_multiple' else 'Gordon Growth')
    ...
    def _valuation_recalculate_from_controls(self, *_: Any, persist: bool = True) -> None:
        ...
        price = metrics.get('price')
        trends = payload.get('trends', {}) if isinstance(payload.get('trends'), dict) else {}
        history_points = int(trends.get('comparable_history_points', 0) or 0)
        material_warnings = []
        if trends.get('per_share_approximate'):
            material_warnings.append('Historical per-share trends use current shares and are approximate.')
    ...
    def _valuation_render_verdict(self, scenarios: dict[str, Any], price: Any) -> None:
        ...
        quality = scenarios.get('model_quality') or scenarios.get('quality') or ('Unavailable' if invalid else 'Medium')
        self.valuation_confidence_label.setText(str(quality))
        upper = scenarios.get('upper_scenario', scenarios.get('bull_fair_value'))
    ...
    def _valuation_render_scenarios(self, scenarios: dict[str, Any]) -> None:
        ...
        for row_index, row in enumerate(rows):
            ...
            assumption_text = (
                ...
                f"{assumptions.get('required_return', assumptions.get('discount_rate', 0)):.1f}% required, "
                f"{assumptions.get('terminal_method', 'Exit Multiple')}"
            )
            values = [
    ...
    def _valuation_render_fair_value_chart(self, payload: dict[str, Any], scenarios: dict[str, Any]) -> None:
        ...
        plot.setLabel('left', 'Value per share')
        ...
        bear = scenarios.get('bear_fair_value', scenario_values.get('bear case'))
        base = scenarios.get('base_fair_value', scenario_values.get('base case'))
        bull = scenarios.get('bull_fair_value', scenario_values.get('bull case'))
        upper = scenarios.get('upper_scenario', bull)
        band_low = self._valuation_float(bear)
        band_high = self._valuation_float(bull)
        if band_low is not None and band_high is not None and band_low <= band_high:
            band_color = QColor(self.theme_color('warning'))
            band_color.setAlpha(34)
            band = pg.LinearRegionItem(values=[band_low, band_high], orientation='horizontal', movable=False, brush=pg.mkBrush(band_color))
            band.setZValue(-10)
            plot.addItem(band)
        for value, token, name, style in (
            (bear, 'accent_negative', 'Bear', Qt.PenStyle.DashLine),
            (base, 'warning', 'Base', Qt.PenStyle.SolidLine),
            (bull, 'accent_positive', 'Bull', Qt.PenStyle.DashLine),
            (scenarios.get('buy_below'), 'accent_positive', 'Buy below', Qt.PenStyle.DotLine),
            (upper, 'accent_negative', 'Upper scenario', Qt.PenStyle.DotLine),
        ):
            numeric = self._valuation_float(value)
            if numeric is not None:
                plot.plot(x_values, [numeric] * len(x_values), pen=self.theme_pen(token, width=1.4, style=style), name=name)

...
class WindowBootstrapMixin:
    ...
    def _portfolio_index_from_id(self, portfolio_id: Any) -> int:
        ...
        portfolio_ids = self._portfolio_selector_ids()
        try:
    ...
    def _portfolio_selector_ids(self) -> list[str]:
        """Return editable portfolio ids followed by the virtual Combined id."""
        return [*self._portfolio_order(), COMBINED_PORTFOLIO_ID]

    def _active_portfolio_is_combined(self) -> bool:
        """Return whether the Portfolio workspace is showing the virtual aggregate."""
    ...
    def _get_portfolio_entry(self, portfolio_id: Any=None) -> Any:
        ...
        if portfolio_id is None:
            ...
            clean_id = str(portfolio_id or '').strip()
            pid = clean_id if clean_id in [*portfolio_order, COMBINED_PORTFOLIO_ID] else portfolio_order[0]
        if pid == COMBINED_PORTFOLIO_ID:
            return build_combined_portfolio_entry(self.all_portfolios_state)
    ...
    def _save_active_options_data(self) -> None:
        ...
        if self._active_portfolio_is_combined():
            return
    ...
    def _persist_all_portfolios(self, *, immediate: bool=False) -> None:
        ...
        if getattr(self, 'main_portfolio_id', None) == COMBINED_PORTFOLIO_ID:
    ...
    def _sync_after_portfolio_change(self, *, refresh_main: bool=False) -> None:
        ...
        self._call_if_page_initialized('_p29_refresh_cards', page_attr='page29', request_data=False)
    ...
    def reorder_portfolios(self, ordered_ids: Any) -> bool:
        """Persist a complete editable portfolio order while preserving identities."""
        current_order = self._portfolio_order()
        requested = []
        if isinstance(ordered_ids, (list, tuple)):
            for value in ordered_ids:
                portfolio_id = str(value or '').strip()
                if portfolio_id == COMBINED_PORTFOLIO_ID:
                    continue
                if portfolio_id in current_order and portfolio_id not in requested:
                    requested.append(portfolio_id)
        if len(requested) != len(current_order) or set(requested) != set(current_order):
            return False
        if requested == current_order:
            return True
        active_id = self.active_portfolio_id
        main_id = self.main_portfolio_id
        self.all_portfolios_state['portfolio_order'] = list(requested)
        self.active_portfolio_id = active_id
        self.main_portfolio_id = main_id
        self.all_portfolios_state['active_portfolio_id'] = active_id
        self.all_portfolios_state['main_portfolio_id'] = main_id
        self._persist_all_portfolios()
        self._sync_after_portfolio_change(refresh_main=False)
        return True
    ...
    def delete_portfolio(self, index: Any) -> bool:
        ...
        selected_ids = self._portfolio_selector_ids()
        delete_index = min(max(delete_index, 0), len(selected_ids) - 1)
        portfolio_id = selected_ids[delete_index]
        if portfolio_id == COMBINED_PORTFOLIO_ID:
            return False
    ...
    def __init__(self, startup_profiler: Any=None, data_service_client: Any=None, startup_progress: Any=None) -> None:
        ...
        with self._startup_profiler_step('window_init'), self._startup_progress_step('window_init', 'Main window'):
            ...
            with self._startup_profiler_step('state_load'), self._startup_progress_step('state_load', 'Saved state'):
                ...
                self.charts_options_top_volume_page_state = load_charts_options_top_volume_page_settings()
                self.backtest_page_state = load_backtest_page_settings()
                self.global_page_state = load_global_page_settings()
                self.up_down_page_state = load_up_down_page_settings()

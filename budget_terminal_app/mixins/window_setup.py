...
class WindowSetupMixin:
    ...
    def _setup_window_shell(self, root_layout: Any) -> None:
        ...
        self.btn_page34 = QPushButton('News')
        ...
        self.btn_page30 = QPushButton('Price')
        ...
        self.btn_page28 = QPushButton('Projections')
        self.btn_page28.setCheckable(True)
    ...
    def _lazy_page_specs(self) -> tuple[dict[str, Any], ...]:
        ...
        return (
            ...
            {'index': 26, 'page_attr': 'page27', 'init_method': 'init_page27'},
    ...
    def _startup_page_labels(self) -> tuple[tuple[int, str], ...]:
        """Return the authoritative live pages tracked by startup progress."""
        labels = [(0, self._PAGE_LABELS.get(0, 'Dashboard'))]
        labels.extend(
    ...
    def _build_page_now(self, index: Any, *, reason: str='lazy') -> Any:
        ...
        if entry.get('building'):
            logger.info('Page load already in progress: %s (index %s).', self._page_label(index), index)
            return None
        ...
        try:
            ...
            if callable(rollback_hook):
                try:
                    ...
                    logger.exception('Lazy-page rollback cleanup failed: %s (index %s).', page_label, page_index)
    ...
    def _hydrate_lazy_page34(self) -> None:
        """Populate News from the most recent dashboard payload after lazy init."""
        if isinstance(getattr(self, 'last_data', None), dict):
            self.update_page34(self.last_data)

...
DEFAULT_CHARTS_OPTIONS_TOP_VOLUME_PAGE_SETTINGS = {
    'symbol': 'SPY',
    'timeframe_label': '1 Day',
    'type_filter': 'calls',
...
COMBINED_PORTFOLIO_ID = 'combined'
COMBINED_PORTFOLIO_NAME = 'Combined'
...
def _portfolio_payload_with_chart_slots(data: Any, chart_slots: Any=None) -> Any:
    ...
    slots = list(chart_slots) if isinstance(chart_slots, (list, tuple)) and chart_slots else list(DEFAULT_CHART_SLOTS)
    if isinstance(data, dict):
        portfolio = data.get('portfolio', [])
        saved_slots = data.get('chart_slots')
        safe_portfolio = list(portfolio) if isinstance(portfolio, (list, tuple)) else []
        safe_slots = list(saved_slots) if isinstance(saved_slots, (list, tuple)) and saved_slots else slots
        return {'portfolio': safe_portfolio, 'chart_slots': safe_slots}
...
def _normalize_selected_portfolio_id(value: Any, portfolio_order: Any, fallback: Any=None) -> Any:
    ...
    fallback_text = str(fallback or (order[0] if order else DEFAULT_MAIN_PORTFOLIO_ID)).strip()
    fallback_id = COMBINED_PORTFOLIO_ID if fallback_text == COMBINED_PORTFOLIO_ID else _normalize_portfolio_id(fallback_text)
    value_text = str(value or '').strip()
    portfolio_id = COMBINED_PORTFOLIO_ID if value_text == COMBINED_PORTFOLIO_ID else _normalize_portfolio_id(value_text)
...
def build_combined_portfolio_entry(state: Any) -> dict[str, Any]:
    """Build the virtual read-only aggregate of every persisted portfolio."""
    payload = state if isinstance(state, dict) else {}
    portfolios = payload.get('portfolios', {}) if isinstance(payload.get('portfolios', {}), dict) else {}
    order = _normalize_portfolio_order(payload.get('portfolio_order'), portfolios)
    tickers = []
    tracker = {}
    options = []
    cash_balance = 0.0
    ...
    for portfolio_id in order:
        ...
        if not isinstance(entry, dict):
            continue
        entry_tracker = entry.get('portfolio_tracker', {})
        if not isinstance(entry_tracker, dict):
            entry_tracker = {}
        for raw_ticker in entry.get('portfolio', []):
            ticker = str(raw_ticker or '').strip().upper()
            if not ticker:
                continue
            if ticker not in tickers:
                tickers.append(ticker)
            source = entry_tracker.get(raw_ticker, entry_tracker.get(ticker))
            if source is None:
                source = next(
                    (
                        tracker_entry for tracker_ticker, tracker_entry in entry_tracker.items()
                        if str(tracker_ticker or '').strip().upper() == ticker
                    ),
                    {},
                )
            source = source if isinstance(source, dict) else {}
            try:
                shares = float(source.get('shares', 0.0) or 0.0)
            except (TypeError, ValueError):
                shares = 0.0
            try:
                avg_price = float(source.get('avg_price', 0.0) or 0.0)
            except (TypeError, ValueError):
                avg_price = 0.0
            ...
            if not math.isfinite(avg_price):
                avg_price = 0.0
            current = tracker.setdefault(ticker, {'shares': 0.0, 'avg_price': 0.0, 'include_in_weight': True})
            old_shares = float(current.get('shares', 0.0) or 0.0)
            old_cost = old_shares * float(current.get('avg_price', 0.0) or 0.0)
            total_shares = old_shares + shares
            total_cost = old_cost + shares * avg_price
            current['shares'] = total_shares
            current['avg_price'] = total_cost / total_shares if total_shares else 0.0
            current['include_in_weight'] = True

        raw_options = entry.get('options_tracker', [])
        if isinstance(raw_options, list):
            for option_index, raw_option in enumerate(raw_options):
                if not isinstance(raw_option, dict):
                    continue
                option = dict(raw_option)
                source_row_id = str(option.get('row_id', '') or option_index)
...
def _normalize_up_down_page_settings(settings: Any) -> dict[str, Any]:
    ...
    if interval_key not in {'1d', '5d', '30d', 'ytd', '1y'}:
        interval_key = DEFAULT_UP_DOWN_PAGE_SETTINGS['interval_key']
    return {
        'active_source': active_source,
        'interval_key': interval_key,
        'custom_symbols': _normalize_unique_symbol_list(saved.get('custom_symbols', DEFAULT_UP_DOWN_PAGE_SETTINGS['custom_symbols'])),
    }
...
def normalize_navigation_settings(settings: Any) -> dict[str, Any]:
    ...
    for page_index in DEFAULT_NAVIGATION_PAGE_ORDER:
        if page_index not in order:
            if page_index == 25 and 0 in order:
            ...
            elif page_index == 26 and 19 in order:
                ...
                order.append(page_index)
    hidden_pages = []
    raw_hidden = saved.get('hidden_pages', DEFAULT_NAVIGATION_SETTINGS['hidden_pages'])
...
def load_active_portfolio_state(portfolio_id: Any=None) -> Any:
    ...
    active = build_combined_portfolio_entry(state) if active_id == COMBINED_PORTFOLIO_ID else state['portfolios'][active_id]
    return {
...
def save_tickers(portfolio: Any, chart_slots: Any=None, portfolio_id: Any=None) -> None:
    ...
    state = load_all_portfolios_state()
    active_id = _resolve_state_portfolio_id(state, portfolio_id or state.get('active_portfolio_id') or state['main_portfolio_id'])
    if active_id == COMBINED_PORTFOLIO_ID:
        return
...
def build_ai_user_data_export() -> str:
    ...
    lines.extend([
        ...
        '',
        '## Up/Down Page Settings',
        '',
        _json_block(payload.get('up_down_page', DEFAULT_UP_DOWN_PAGE_SETTINGS)),
        '',
        '## Compare Presets',
        '',
...
def save_app_config(data: Any) -> None:
    ...
    if 'charts_options_top_volume_page' in saved:
        current['charts_options_top_volume_page'] = _normalize_charts_options_top_volume_page_settings(saved.get('charts_options_top_volume_page'))
...
def _normalize_chart_drawing_context_key(value: Any) -> str:
    """Return a normalized SYMBOL|INTERVAL drawing context key."""
    text = str(value or '').strip()
    if '|' not in text:
        return ''
    symbol, interval_label = text.split('|', 1)
    symbol = str(symbol or '').upper().strip()
    interval_label = str(interval_label or '').strip()
    if not symbol or interval_label not in _CHART_INTERVAL_RANGE_OPTIONS:
        return ''
    return f'{symbol}|{interval_label}'


def _normalize_chart_drawing_anchor(value: Any) -> dict[str, Any] | None:
    """Normalize a timestamp/price drawing anchor."""
    ...
    if getattr(parsed, 'tzinfo', None) is not None:
        try:
            ...
            try:
                ...
                pass
    return {'time': parsed.isoformat(), 'price': price}


def _normalize_chart_drawing_record(value: Any) -> dict[str, Any] | None:
    """Normalize one persisted native chart drawing record."""
    if not isinstance(value, dict):
        return None
    drawing_id = str(value.get('id') or '').strip()
    drawing_type = str(value.get('type') or '').strip().lower()
    if not drawing_id or drawing_type not in _CHART_DRAWING_TYPES:
        return None
    anchors = []
    for raw_anchor in value.get('anchors', []) if isinstance(value.get('anchors'), list) else []:
        anchor = _normalize_chart_drawing_anchor(raw_anchor)
        if anchor is not None:
            anchors.append(anchor)
    required_anchor_count = 1 if drawing_type in {'horizontal_line', 'horizontal_ray', 'text'} else 2
    if len(anchors) < required_anchor_count:
        return None
    anchors = anchors[:required_anchor_count]
    text = str(value.get('text') or '').strip()[:500] if drawing_type == 'text' else ''
    if drawing_type == 'text' and not text:
        return None
    raw_style = value.get('style', {}) if isinstance(value.get('style'), dict) else {}
    color = str(raw_style.get('color') or '').strip()[:32]
    try:
        width = float(raw_style.get('width', 1.5))
    except (TypeError, ValueError):
        width = 1.5
...
def _normalize_chart_drawings_by_context(value: Any) -> dict[str, list[dict[str, Any]]]:
    """Normalize all persisted native chart drawings."""
    if not isinstance(value, dict):
        return {}
    normalized = {}
    for raw_key, raw_records in value.items():
        key = _normalize_chart_drawing_context_key(raw_key)
        if not key or not isinstance(raw_records, list):
            continue
        records = []
        for raw_record in raw_records:
            record = _normalize_chart_drawing_record(raw_record)
...
def _normalize_chart_page_settings(settings: Any) -> Any:
    ...
    interval_label = str(saved.get('interval_label', timeframe_label) or timeframe_label).strip()
    if interval_label not in _CHART_INTERVAL_RANGE_OPTIONS:
        interval_label = DEFAULT_CHART_PAGE_SETTINGS['interval_label']
    raw_range_label = str(saved.get('range_label', '') or '').strip()
    if raw_range_label.upper() == 'MAX':
        raw_range_label = 'Max'
    allowed_ranges = _CHART_INTERVAL_RANGE_OPTIONS[interval_label]
    range_label = raw_range_label if raw_range_label in allowed_ranges else _CHART_LEGACY_RANGE_DEFAULTS[interval_label]
    chart_type = str(saved.get('chart_type', DEFAULT_CHART_PAGE_SETTINGS['chart_type']) or '').strip().lower()
    if chart_type not in {'candles', 'line', 'area'}:
        chart_type = DEFAULT_CHART_PAGE_SETTINGS['chart_type']
    try:
        poll_interval_seconds = int(saved.get('poll_interval_seconds', DEFAULT_CHART_PAGE_SETTINGS['poll_interval_seconds']))
    except (TypeError, ValueError):
        poll_interval_seconds = DEFAULT_CHART_PAGE_SETTINGS['poll_interval_seconds']
    if poll_interval_seconds not in {0, 15, 30, 60}:
        poll_interval_seconds = DEFAULT_CHART_PAGE_SETTINGS['poll_interval_seconds']
    sidebar_value = saved.get('sidebar_visible', DEFAULT_CHART_PAGE_SETTINGS['sidebar_visible'])
    sidebar_visible = bool(sidebar_value) if isinstance(sidebar_value, bool | int) else DEFAULT_CHART_PAGE_SETTINGS['sidebar_visible']
    splitter_sizes = _normalize_splitter_sizes(
        saved.get('splitter_sizes'),
        DEFAULT_CHART_PAGE_SETTINGS['splitter_sizes'],
        2,
    )
    drawings_by_context = _normalize_chart_drawings_by_context(saved.get('drawings_by_context', {}))
...
def _normalize_charts_options_top_volume_page_settings(settings: Any) -> Any:
    """Normalize persisted state for the Projections page."""
    saved = settings if isinstance(settings, dict) else {}
    symbol = str(
        saved.get('symbol', DEFAULT_CHARTS_OPTIONS_TOP_VOLUME_PAGE_SETTINGS['symbol'])
        or DEFAULT_CHARTS_OPTIONS_TOP_VOLUME_PAGE_SETTINGS['symbol']
    ).upper().strip()
    timeframe_label = str(
        saved.get('timeframe_label', DEFAULT_CHARTS_OPTIONS_TOP_VOLUME_PAGE_SETTINGS['timeframe_label'])
        or DEFAULT_CHARTS_OPTIONS_TOP_VOLUME_PAGE_SETTINGS['timeframe_label']
    ).strip()
    valid_timeframes = {'1 Minute', '5 Minutes', '15 Minutes', '1 Hour', '4 Hours', '1 Day', '1 Week', '1 Month'}
    if timeframe_label not in valid_timeframes:
        timeframe_label = DEFAULT_CHARTS_OPTIONS_TOP_VOLUME_PAGE_SETTINGS['timeframe_label']
    type_filter = str(
        saved.get('type_filter', DEFAULT_CHARTS_OPTIONS_TOP_VOLUME_PAGE_SETTINGS['type_filter'])
        or DEFAULT_CHARTS_OPTIONS_TOP_VOLUME_PAGE_SETTINGS['type_filter']
    ).strip().lower()
    if type_filter not in {'calls', 'puts'}:
        type_filter = 'calls'
    expiration_scope = 'all'
    splitter_sizes = _normalize_splitter_sizes(
        saved.get('splitter_sizes'),
        DEFAULT_CHARTS_OPTIONS_TOP_VOLUME_PAGE_SETTINGS['splitter_sizes'],
        2,
    )
    return {
        'symbol': symbol or DEFAULT_CHARTS_OPTIONS_TOP_VOLUME_PAGE_SETTINGS['symbol'],
...
def _normalize_valuation_assumptions(settings: Any) -> dict[str, Any]:
    ...
    terminal_method = str(saved.get('terminal_method', default['terminal_method']) or default['terminal_method']).lower().strip()
    if terminal_method not in {'gordon_growth', 'exit_multiple'}:
        terminal_method = default['terminal_method']
    ...
    years = min(max(years, 5), 15)
    return {
        'basis_type': basis_type,
        'terminal_method': terminal_method,
        'basis_value': _bounded(saved.get('basis_value', default['basis_value']), float(default['basis_value']), 0.0, 100000.0),
        'growth_1_5': _bounded(saved.get('growth_1_5', default['growth_1_5']), float(default['growth_1_5']), -30.0, 50.0),
        'growth_6_10': _bounded(saved.get('growth_6_10', default['growth_6_10']), float(default['growth_6_10']), -15.0, 30.0),
        'discount_rate': _bounded(saved.get('discount_rate', default['discount_rate']), float(default['discount_rate']), 4.0, 30.0),
        'terminal_growth': _bounded(saved.get('terminal_growth', default['terminal_growth']), float(default['terminal_growth']), -2.0, 5.0),
        'exit_multiple': _bounded(saved.get('exit_multiple', default['exit_multiple']), float(default['exit_multiple']), 3.0, 50.0),
        'projection_years': years,
        'margin_of_safety': _bounded(saved.get('margin_of_safety', default['margin_of_safety']), float(default['margin_of_safety']), 0.0, 50.0),
    }


def _normalize_valuation_assumptions_by_ticker(values: Any) -> dict[str, dict[str, Any]]:
    """Normalize ticker-scoped Valuation assumptions."""
    normalized: dict[str, dict[str, Any]] = {}
    if not isinstance(values, dict):
        return normalized
    for ticker_key, assumptions in values.items():
        ticker = str(ticker_key or '').upper().strip()
        if ticker and isinstance(assumptions, dict):
            normalized[ticker] = _normalize_valuation_assumptions(assumptions)
    return normalized
...
def _normalize_valuation_page_settings(settings: Any) -> dict[str, Any]:
    ...
    legacy_assumptions = _normalize_valuation_assumptions(saved.get('assumptions', DEFAULT_VALUATION_PAGE_SETTINGS['assumptions']))
    assumptions_by_ticker = _normalize_valuation_assumptions_by_ticker(
        saved.get('assumptions_by_ticker', DEFAULT_VALUATION_PAGE_SETTINGS['assumptions_by_ticker'])
    )
    if last_ticker not in assumptions_by_ticker:
        assumptions_by_ticker[last_ticker] = legacy_assumptions
    current_assumptions = dict(assumptions_by_ticker[last_ticker])
    notes_by_ticker = {}
...
def load_charts_options_top_volume_page_settings() -> Any:
    """Load persisted state for the Projections page."""
    config = load_app_config()
    return _normalize_charts_options_top_volume_page_settings(config.get('charts_options_top_volume_page', {}))


def save_charts_options_top_volume_page_settings(settings: Any) -> Any:
    """Persist state for the Projections page."""
    current = load_app_config()
    state = _normalize_charts_options_top_volume_page_settings(settings)
    current['charts_options_top_volume_page'] = state
    save_app_config(current)
    return state
...
def load_up_down_page_settings() -> Any:
    """Load persisted state for the standalone Up/Down page."""
    config = load_app_config()
    return _normalize_up_down_page_settings(config.get('up_down_page', {}))


def save_up_down_page_settings(settings: Any) -> Any:
    """Persist state for the standalone Up/Down page."""
    current = load_app_config()
    state = _normalize_up_down_page_settings(settings)
    current['up_down_page'] = state
    save_app_config(current)
    return state

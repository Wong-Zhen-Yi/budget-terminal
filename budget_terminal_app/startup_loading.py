...
class StartupLoadingScreen(QDialog):
    ...
    def register_pages(self, page_labels: Any) -> None:
        ...
        stale_keys = {
            key for key in self._page_keys
            if key.startswith('page_') and key not in expected_keys
        }
        for key in stale_keys:
            row = self._task_rows.pop(key, None)
            self._task_bars.pop(key, None)
            self._task_labels.pop(key, None)
        ...
        for index, label in normalized_pages:
            key = self._page_key(index)
            self.register_task(key, label, section='pages')
            task_label = self._task_labels.get(key)
    ...
    def request_skip(self) -> None:
        ...
        self.status_label.setText('Opening now; remaining startup work will continue in the background...')
        self._pump_events()
    ...
    def required_startup_complete(self) -> bool:
        for key in REQUIRED_STARTUP_TASK_KEYS:
            ...
            if bar is None or bar.value() < 100:
                return False
        if not self.all_pages_complete():
            return False
        return True

...
from ..services.news_analysis import (
    ...
    article_ticker_set,
    dedupe_articles,
    mentions_blocked_ticker,
    normalize_news_symbol,
)
from ..services.technical_analysis import calculate_rsi
from .news_sources import fetch_keyless_trader_news_with_status
...
class DataWorker(QObject):
    ...
    _other_news_meta_cache: dict[str, Any] = {}
    ...
    def _normalize_chart_configs(self) -> list[tuple[str, Any, Any]]:
        return normalize_chart_configs(self.chart_configs)

    def _dedupe_symbols(self, *groups: Any) -> list[str]:
        return dedupe_symbols(*groups)
    ...
    def _extract_close_series(self, batch_data: Any, all_symbols: list[str], symbol: str) -> Any:
        return extract_close_series(batch_data, all_symbols, symbol)
    ...
    def _build_price_payload(self, close: Any) -> Any:
        return price_payload(close)

    def _build_market_payload(self, close: Any) -> Any:
        return market_payload(close)
    ...
    def _normalize_news_symbol(value: Any) -> str:
        ...
        return normalize_news_symbol(value)
    ...
    def _article_dedupe_key(self, article: dict[str, Any]) -> str:
        ...
        return article_dedupe_key(article)
    ...
    def _dedupe_news(self, articles: list[dict[str, Any]], existing: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        return dedupe_articles(articles, existing)
    ...
    def _fetch_stock_details(self, ticker: str, portfolio_info: dict[str, Any]) -> Any:
        ...
        try:
            ...
            quote_type = str(fast_info.get('quoteType') or '').upper().strip()
        except Exception as exc:
            logger.info('Quote type metadata unavailable for %s: %s', ticker, exc)
        unavailable_reason = ''
        if quote_type == 'ETF':
            unavailable_reason = 'not_applicable_for_etf'
        else:
            try:
                ...
                if is_yahoo_unauthorized_error(exc):
                    ...
                    logger.warning('Target metadata fetch failed for equity %s: %s', ticker, exc)
        portfolio_news, _news_error = self._fetch_portfolio_news_with_status(ticker)
        payload = {
            'targets': self._target_payload(
                ticker,
                portfolio_info.get(ticker, {}).get('price', 0),
                info,
    ...
    def fetch_ticker_news_only(self, max_per_ticker: int = 3) -> dict[str, Any]:
        """Fetch exact-associated news for an ad hoc ticker universe."""
        limit = max(1, min(int(max_per_ticker), 10))
        queried_tickers = self._dedupe_symbols(
            [str(ticker or '').upper().strip() for ticker in self.tickers]
        )
        news_list: list[dict[str, Any]] = []
        empty_tickers: list[str] = []
        failed_tickers: list[str] = []
        if not queried_tickers or self._is_cancelled():
            return {
                'queried_tickers': queried_tickers,
                'articles': [],
                'empty_tickers': empty_tickers,
                'failed_tickers': failed_tickers,
            }
        with ThreadPoolExecutor(max_workers=self._executor_workers(len(queried_tickers))) as executor:
            results = executor.map(self._fetch_portfolio_news_with_status, queried_tickers)
            for ticker, result in zip(queried_tickers, results):
                if self._is_cancelled():
                    break
                articles, error = result
                if error:
                ...
                for article in ranked[:limit]:
                    ...
                    news_list.append(searched_article)
        deduped = self._dedupe_news(news_list, [])
        deduped.sort(key=lambda article: float(article.get('_ts') or 0), reverse=True)
        return {
            'queried_tickers': queried_tickers,
    ...
    def _fetch_other_news(self) -> list[dict[str, Any]]:
        """Fetch general-market news while preserving the legacy list interface."""
        return self._fetch_other_news_with_status()['articles']

    def _fetch_other_news_with_status(self) -> dict[str, Any]:
        """Fetch general-market news and report cache, stale, and failed source state."""
        now = time.time()
        with self._details_cache_lock:
            cached = self._other_news_cache
            if cached and (now - cached[0]) < self._DETAILS_CACHE_TTL_SECONDS:
                cached_meta = dict(self._other_news_meta_cache)
                cached_meta['articles'] = [dict(item) for item in cached[1]]
                cached_meta['cache_hit'] = True
                return cached_meta
        result = fetch_keyless_trader_news_with_status(
        ...
        status = {
            ...
            'fresh_sources': list(result.get('fresh_sources', [])),
            'stale_sources': list(result.get('stale_sources', [])),
            'failed_sources': list(result.get('failed_sources', [])),
        ...
        with self._details_cache_lock:
            ...
            self.__class__._other_news_meta_cache = {
                key: value
    ...
    def _collect_portfolio_news_page(self) -> tuple[list[dict[str, Any]], list[str]]:
        """Collect portfolio news plus ticker-level failures for a News-page refresh."""
        articles: list[dict[str, Any]] = []
        failed_tickers: list[str] = []
        if not self.tickers:
            return articles, failed_tickers
        with ThreadPoolExecutor(max_workers=self._executor_workers(len(self.tickers))) as executor:
            results = executor.map(self._fetch_portfolio_news_with_status, self.tickers)
            for ticker, result in zip(self.tickers, results):
                if self._is_cancelled():
                    return [], []
                ticker_articles, error = result
                articles.extend(ticker_articles)
                if error:
                    failed_tickers.append(str(ticker))
        return articles, failed_tickers

    def _collect_macro_news_page(self) -> list[dict[str, Any]]:
        """Collect macro news for a News-page refresh."""
        articles: list[dict[str, Any]] = []
        with ThreadPoolExecutor(max_workers=self._executor_workers(len(MACRO_TICKERS))) as executor:
            for ticker_articles in executor.map(self._fetch_macro_news, MACRO_TICKERS):
                if self._is_cancelled():
                    return []
                articles.extend(ticker_articles)
        return articles
    ...
    def _collect_news_page_payload(self) -> dict[str, Any] | None:
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            portfolio_future = executor.submit(self._collect_portfolio_news_page)
            macro_future = executor.submit(self._collect_macro_news_page)
            other_future = executor.submit(self._fetch_other_news_with_status)
            portfolio_news, failed_tickers = portfolio_future.result()
            macro_news = macro_future.result()
            other_result = other_future.result()
        if self._is_cancelled():
            return None
        news_list = list(portfolio_news) + list(macro_news)
        other_news = self._filter_other_news(list(other_result.get('articles', [])), news_list)
        news_list.extend(other_news)
        return {
            'news': news_list,
            '_news_refresh_meta': {
                'failed_tickers': list(failed_tickers),
                'fresh_sources': list(other_result.get('fresh_sources', [])),
                'stale_sources': list(other_result.get('stale_sources', [])),
                'failed_sources': list(other_result.get('failed_sources', [])),
                'other_news_cache_hit': bool(other_result.get('cache_hit', False)),
            },
        }
    ...
    def _fetch_news_page_refresh(self, total_started: float) -> dict[str, Any] | None:
        ...
        payload = {
            ...
            '_news_refresh_meta': dict((news_payload or {}).get('_news_refresh_meta', {})),
            'charts': {},
            'chart_options': {},
    ...
    def _calculate_rsi(close_series: Any, period: int = 14) -> Any:
        ...
        return calculate_rsi(close_series, period)

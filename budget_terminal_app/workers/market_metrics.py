...
class MarketCapWorker(QObject):
    ...
    _ETF_AUM_CACHE_LOCK = threading.Lock()
    ...
    def fetch(self) -> dict[str, Any]:
        """Return typed equity market-cap or ETF AUM metadata synchronously."""
        try:
            results = {}
            ...
            def fetch_mktcap(t: Any) -> Any:
                ...
                try:
                    ...
                    if quote_type == 'ETF':
                        return (t, {'symbol': t, 'quote_type': 'ETF', 'size_type': 'aum', 'size_value': None})
                    if mc:
                        return (t, {'symbol': t, 'quote_type': quote_type or 'EQUITY', 'size_type': 'market_cap', 'size_value': mc})
                    with YF_LOCK:
                        try:
                            info = ticker.info
                    ...
                    return (t, {
                        'symbol': t,
                        'quote_type': str(info.get('quoteType') or quote_type or 'EQUITY').upper(),
                        'size_type': 'market_cap',
                        'size_value': info.get('marketCap'),
                    })
                except Exception as ex:
                    logger.warning(f'MarketCap fetch error {t}: {ex}')
                    return (t, {'symbol': t, 'quote_type': '', 'size_type': 'market_cap', 'size_value': None})
            ...
            for t, mc in res_list:
                results[t] = mc
            etf_symbols = [
                str(symbol).upper() for symbol, payload in results.items()
                if isinstance(payload, dict) and payload.get('quote_type') == 'ETF'
            ]
            if etf_symbols:
                aum_values = self._fetch_etf_aum(etf_symbols)
                for symbol in etf_symbols:
                    results[symbol]['size_value'] = aum_values.get(symbol)
            return results
        except Exception as ex:
    ...
    @classmethod
    def _fetch_etf_aum(cls, symbols: list[str]) -> dict[str, float]:
        """Load requested ETF AUM values from Yahoo's ETF screener with a daily cache."""
        requested = {str(symbol or '').upper().strip() for symbol in symbols if str(symbol or '').strip()}
        now = time.time()
        with cls._ETF_AUM_CACHE_LOCK:
            cache_is_fresh = (now - cls._ETF_AUM_CACHE_AT) < cls._ETF_AUM_CACHE_TTL_SECONDS
            if cache_is_fresh:
                return {
                    symbol: cls._ETF_AUM_CACHE[symbol]
                    for symbol in requested
                    if symbol in cls._ETF_AUM_CACHE
                }
        ...
        try:
            ...
            if etf_query is None:
                return {}
            query = etf_query('gt', ['fundnetassets', 0])
            offset = 0
            while requested - loaded.keys():
                with YF_LOCK:
                    response = yf.screen(
                        query,
                        offset=offset,
                ...
                quotes = list(response.get('quotes') or []) if isinstance(response, dict) else []
                if not quotes:
                    break
                for quote in quotes:
                    ...
                    raw_value = quote.get('netAssets') or quote.get('fundnetassets')
                    try:
            ...
            logger.warning('ETF AUM screener unavailable; keeping cached values: %s', exc)
        with cls._ETF_AUM_CACHE_LOCK:
            if loaded:
                cls._ETF_AUM_CACHE.update(loaded)
            cls._ETF_AUM_CACHE_AT = now
            return {
                symbol: cls._ETF_AUM_CACHE[symbol]
                for symbol in requested
                if symbol in cls._ETF_AUM_CACHE

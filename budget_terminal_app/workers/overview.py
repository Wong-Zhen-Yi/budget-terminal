...
class TradingVolumeWorker(QObject):
    ...
    def _merge_trading_volume_history(self, rows: list[dict[str, Any]]) -> None:
        ...
        for row in rows:
            ...
            for key, value in metrics.items():
                if key == 'one_day_price_return_pct' and row.get(key) is not None:
                    continue
    ...
    def _dollar_volume_metrics(self, frame: Any) -> dict[str, float | None]:
        ...
        try:
            ...
            if not getattr(volume, 'empty', True):
                volume.index = pd.DatetimeIndex(pd.to_datetime(volume.index, errors='coerce')).tz_localize(None)
                volume = volume[pd.notna(volume.index)]
    ...
    def _trailing_price_return_pct(self, close: Any, periods: int) -> float | None:
        ...
        try:
            if len(close) <= periods:
                return None
            return self._price_return_pct(float(close.iloc[-(periods + 1)]), float(close.iloc[-1]))
        except Exception:
            return None

    def _window_price_return_pct(self, close: Any) -> float | None:
        if getattr(close, 'empty', True):
            return None
        try:
            return self._price_return_pct(float(close.iloc[0]), float(close.iloc[-1]))
        except Exception:
            return None

    def _price_return_pct(self, start_price: Any, end_price: Any) -> float | None:
        try:

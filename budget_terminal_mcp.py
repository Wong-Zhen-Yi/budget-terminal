"""Launch Budget Terminal as a Windows-friendly stdio MCP server."""
...
_LOCAL_VENV_REEXEC_ENV = 'BUDGET_TERMINAL_MCP_LOCAL_VENV_REEXECED'
...
def _maybe_reexec_from_local_venv() -> int | None:
    ...
    if venv_python is None or _same_executable(Path(sys.executable), venv_python):
        return None
    env = dict(os.environ)
...
from budget_terminal_app.mcp.__main__ import main

...
from budget_terminal_app.mixins.charts_page import P10_CURSOR_LABEL, P10_FIB_RETRACEMENT_LABEL
...
def test_chart_indicator_persistence_normalization() -> None:
    ...
    assert _normalize_chart_page_settings({'indicators': ['Crosshair']})['indicators'] == [P10_CURSOR_LABEL]
...
def test_offscreen_chart_startup_indicators_honor_subset() -> None:
    ...
    try:
        ...
        assert not window._p10_indicator_buttons[P10_CURSOR_LABEL].isChecked()
    finally:
        window.close()
        window.deleteLater()
        app.processEvents()
...
def test_cursor_indicator_toggle_persists() -> None:
    import budget_terminal_app.mixins.charts_page as charts_page_module
    ...
    try:
        ...
        try:
            ...
            assert saved_states
            assert P10_CURSOR_LABEL in saved_states[-1]['indicators']
        finally:
            window.close()
            window.deleteLater()
            app.processEvents()

        app, window = _build_window(_normalize_chart_page_settings(saved_states[-1]))
        try:
            assert P10_CURSOR_LABEL in window.p10_active_indicators
            assert window._p10_indicator_buttons[P10_CURSOR_LABEL].isChecked()
        finally:
            window.close()
            window.deleteLater()
            app.processEvents()
...
if __name__ == '__main__':
    ...
    test_cursor_indicator_toggle_persists()

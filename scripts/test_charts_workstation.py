...
from budget_terminal_app.mixins.charts_page import P10_CURSOR_LABEL
...
def test_native_drawing_previews() -> None:
    from budget_terminal_app.main import QApplication
    ...
    changes: list[list[dict]] = []
    palette = {'accent': '#5aa2ff', 'warning': '#ffbd5a'}
    controller = NativeChartDrawingController(
        plot,
        dates=lambda: dates,
        theme_color=lambda token: palette[token],
        changed=lambda records: changes.append(records),
        request_text=lambda: 'Note',
    )
    expected_item_counts = {'trend_line': 2, 'rectangle': 2, 'fib': 9}
    for tool, expected_count in expected_item_counts.items():
        controller.set_records([])
        changes.clear()
        controller.set_tool(tool)
        assert controller.handle_click(4.4, 100.0)
        assert controller.pending_anchor is not None
        assert controller.records_payload() == []
        assert controller.items == {}
        assert controller.selected_id is None
        assert len(controller._preview_items) == expected_count
        assert controller._point_from_anchor(controller.pending_anchor) == (4.0, 100.0)
        assert not controller._undo and not controller._redo and not changes

        assert controller.update_preview(12.6, 111.5)
        assert controller._preview_endpoint is not None
        assert controller._point_from_anchor(controller._preview_endpoint) == (13.0, 111.5)
        assert len(controller._preview_items) == expected_count
        if tool == 'trend_line':
            old_items = list(controller._preview_items)
            palette['warning'] = '#00cc88'
            controller.apply_theme()
            preview_pen = controller._preview_items[1].opts['pen']
            assert preview_pen.color().name().lower() == '#00cc88'
            assert controller._preview_items != old_items
            assert controller._point_from_anchor(controller._preview_endpoint) == (13.0, 111.5)

        assert controller.handle_click(12.6, 111.5)
        assert len(controller.records) == 1
        assert controller.records[0]['type'] == tool
        assert len(controller.records[0]['anchors']) == 2
        assert controller.active_tool == 'cursor'
        assert controller.pending_anchor is None
        assert controller._preview_endpoint is None
        assert controller._preview_items == []
        assert len(controller._undo) == 1 and not controller._redo
        assert len(changes) == 1

    controller.set_records([])
    controller.set_tool('trend_line')
    controller.handle_click(5, 101)
    controller.set_tool('rectangle')
    assert controller.active_tool == 'rectangle'
    assert controller.pending_anchor is None and not controller._preview_items
    controller.handle_click(6, 102)
    controller.set_records([])
    assert controller.active_tool == 'cursor'
    assert controller.pending_anchor is None and not controller._preview_items
    controller.set_tool('fib')
    controller.handle_click(7, 103)
...
def test_offscreen_main_workstation_smoke() -> None:
    ...
    try:
        assert window.p10_chart_type == 'candles'
        assert window.p10_chart_type_combo.currentData() == 'candles'
        frame = _sample_frame()
        ...
        window._p10_refresh_chart = lambda force_refresh=False: refresh_calls.append(bool(force_refresh))
        range_widths = []
        for label in ('1Y', '6M', '3M', '1M'):
            window._p10_set_range(label)
            fitted = window._p10_get_current_x_range()
            range_widths.append(fitted[1] - fitted[0])
        assert all(left > right for left, right in zip(range_widths, range_widths[1:]))
        assert refresh_calls == []

        window.p10_auto_follow = False
        window._p10_manual_x_range = (10.0, 30.0)
        window._p10_set_range('3M')
        fitted_manual = window._p10_get_current_x_range()
        assert window._p10_manual_x_range == fitted_manual
        window._p10_set_x_range((25.0, 55.0))
        window._p10_manual_x_range = window._p10_get_current_x_range()
        window._p10_set_range('3M')
        assert window._p10_get_current_x_range() == fitted_manual
        assert refresh_calls == []

        window._p10_set_range('5Y')
        available_range = window._p10_get_current_x_range()
        assert available_range[0] <= 0.0
        assert refresh_calls == [False]
        assert window._p10_force_selected_range is True
        window._p10_set_range('Max')
        assert refresh_calls == [False, True]
        window._p10_set_range('Max')
        assert refresh_calls == [False, True]
        window.p10_auto_follow = True
        window._p10_set_range('3M')
        window._p10_refresh_chart = original_refresh

        assert window.p10_drawing_tools_label.text() == 'Drawing Tools'
        assert window.p10_drawing_tools_label.alignment() & pg.QtCore.Qt.AlignmentFlag.AlignHCenter
        assert window.p10_drawing_rail_widget.width() == 88
        assert 'cursor' not in window._p10_drawing_tool_buttons
        window._p10_select_drawing_tool('trend_line')
        assert window.p10_drawing_controller.active_tool == 'trend_line'
        x_range, y_range = window.p10_main_plot.getPlotItem().vb.viewRange()
        anchor_x = (x_range[0] + x_range[1]) / 2.0
        anchor_y = (y_range[0] + y_range[1]) / 2.0
        assert window.p10_drawing_controller.handle_click(anchor_x, anchor_y)
        assert window.p10_drawing_controller._preview_items
        preview_x = anchor_x + 3.0
        preview_y = anchor_y + 1.0
        preview_pos = window.p10_main_plot.getPlotItem().vb.mapViewToScene(
            pg.QtCore.QPointF(preview_x, preview_y)
        )
        assert P10_CURSOR_LABEL not in window.p10_active_indicators
        window._p10_on_mouse_moved(window.p10_main_plot, [preview_pos])
        preview_endpoint = window.p10_drawing_controller._preview_endpoint
        assert preview_endpoint is not None
        endpoint_point = window.p10_drawing_controller._point_from_anchor(preview_endpoint)
        assert endpoint_point is not None and endpoint_point[0] == round(preview_x)
        window._p10_select_drawing_tool('trend_line')
        assert window.p10_drawing_controller.active_tool == 'cursor'
        assert window.p10_drawing_controller.pending_anchor is None
        assert not window.p10_drawing_controller._preview_items
        assert not any(button.isChecked() for button in window._p10_drawing_tool_buttons.values())
        window._p10_select_drawing_tool('rectangle')
        assert window.p10_drawing_controller.handle_click(anchor_x, anchor_y)
        assert window.p10_drawing_controller._preview_items
        window._p10_select_drawing_tool('fib')
        assert window.p10_drawing_controller.active_tool == 'fib'
        assert window.p10_drawing_controller.pending_anchor is None
        assert not window.p10_drawing_controller._preview_items
        window._p10_escape_chart_action()
        assert window.p10_drawing_controller.active_tool == 'cursor'
        assert not any(button.isChecked() for button in window._p10_drawing_tool_buttons.values())
        ...
        assert not any(line.isVisible() for line in window.p10_crosshair_v_lines)
        assert not window.p10_crosshair_h_line.isVisible()
        assert not window.p10_crosshair_time_label.isVisible()

        window._p10_toggle_indicator(P10_CURSOR_LABEL, True)
        window._p10_on_mouse_moved(window.p10_main_plot, [scene_pos])
        ...
        window._p10_toggle_indicator(P10_CURSOR_LABEL, False)
        assert not any(line.isVisible() for line in window.p10_crosshair_v_lines)
        assert not window.p10_crosshair_h_line.isVisible()
        assert not window.p10_crosshair_time_label.isVisible()

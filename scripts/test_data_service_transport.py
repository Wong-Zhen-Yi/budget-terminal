...
def test_coordinator_uses_one_inline_provider_call_without_timeout_retry() -> None:
    coordinator = DashboardFetchCoordinator(max_workers=2)
    assert coordinator._task_runner.default_timeout_seconds == 0.0
    assert coordinator._task_runner.default_retries == 0
    call_count = 0
    release = threading.Event()

    def slow_fetch() -> dict[str, int]:
        nonlocal call_count
        call_count += 1
    ...
    try:
        with ThreadPoolExecutor(max_workers=2) as executor:
            first = executor.submit(coordinator._coalesced_fetch, "slow_test", (("AAPL",),), slow_fetch)
            second = executor.submit(coordinator._coalesced_fetch, "slow_test", (("AAPL",),), slow_fetch)
            time.sleep(0.05)
            release.set()
            assert first.result(timeout=5)["value"] == 7
            assert second.result(timeout=5)["value"] == 7
    finally:
        release.set()
        coordinator.shutdown(wait=True)
    assert call_count == 1
...
if __name__ == "__main__":
    ...
    test_coordinator_uses_one_inline_provider_call_without_timeout_retry()

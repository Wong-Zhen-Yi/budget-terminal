...
def test_calendar_startup_defaults_to_reference_today() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        original_path = _use_temp_user_data(Path(tmp))
        try:
            from budget_terminal_app.app import BudgetTerminalApp
            ...
            try:
                ...
                reference_today = datetime.date(2026, 7, 13)
                timezone_index = 1
                _assert(window.p7_tz_combo.count() > timezone_index, "Calendar should expose a non-local reference timezone")
                window._now_for_timezone_index = lambda index: datetime.datetime.combine(
                    reference_today,
                    datetime.time(12, 0),
                    tzinfo=datetime.timezone.utc,
                )
                rendered_months: list[tuple[int, int]] = []
                window._p7_render_month = lambda: rendered_months.append((window._p7_year, window._p7_month))
                window._p7_save_session_snapshot = lambda **kwargs: None

                window._p7_restore_startup_session(
                    {
                        "events": {"TEST": {"earnings": "2026-07-20"}},
                        "year": 2026,
                        "month": 8,
                ...
                _assert(window.p7_tz_combo.currentIndex() == timezone_index, "startup should restore the reference timezone")
                _assert("TEST" in window._p7_events, "startup should restore cached Calendar events")
                _assert(
                    (window._p7_year, window._p7_month) == (reference_today.year, reference_today.month),
                    "startup should ignore the stale saved month and display the reference timezone's current month",
                )
                _assert(
                    rendered_months[-1] == (reference_today.year, reference_today.month),
                    "startup should render the current month after restoring the timezone",
                )

                window._p7_change_month(1)
                _assert((window._p7_year, window._p7_month) == (2026, 8), "next-month navigation should remain functional")
                window._p7_change_month(-1)
                _assert((window._p7_year, window._p7_month) == (2026, 7), "previous-month navigation should remain functional")
                window._p7_change_month(1)
                window._p7_jump_to_present()
                _assert(
                    (window._p7_year, window._p7_month) == (reference_today.year, reference_today.month),
                    "Today should still return the Calendar to the reference timezone's current month",
                )
            finally:
                WindowLifecycleMixin._schedule_startup_refresh = original_schedule_startup_refresh
                WindowLifecycleMixin._start_lazy_warmup = original_start_lazy_warmup
...
def test_calendar_monthly_responsive_layout() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        original_path = _use_temp_user_data(Path(tmp))
        try:
            from budget_terminal_app.app import BudgetTerminalApp
            from budget_terminal_app.dependencies import Qt
            from budget_terminal_app.main import QApplication
            from budget_terminal_app.mixins.calendar_page import CalendarPageMixin
            from budget_terminal_app.mixins.window_lifecycle import WindowLifecycleMixin
            ...
            try:
                ...
                app.processEvents()
                window.p7_tabs.setCurrentWidget(window.p7_calendar_tab)
                window.p7_calendar_scroll.resize(820, 600)
                window._p7_apply_monthly_responsive_layout()
                app.processEvents()

                _assert(
                    window.p7_calendar_scroll.widget() is window.p7_calendar_content,
                    "monthly Calendar content should live inside the Calendar scroll area",
                )
                _assert(
                    window.p7_details_splitter.orientation() == Qt.Orientation.Vertical,
                    "monthly Calendar details should stack vertically in compact mode",
                )
                _assert(window.p7_today_btn.text() == "Today", "compact Calendar button should be labeled Today")
                _assert(window.p7_export_llm_btn.text() == "Export", "compact Calendar export button should be labeled Export")

                window._p7_apply_detail_table_widths()
                for table_name in (
                    "p7_company_events_table",
                    "p7_economic_events_table",
                    "p7_market_holidays_table",
                    "p7_options_exp_table",
                ):
                    table = getattr(window, table_name)
                    _assert(table.columnWidth(0) > 0, f"{table_name} should keep a non-zero first column width")

                window.resize(1040, 680)
                window.p7_calendar_scroll.resize(1040, 600)
                app.processEvents()
                window._p7_apply_monthly_responsive_layout()
                _assert(
                    window.p7_details_splitter.orientation() == Qt.Orientation.Horizontal,
                    "monthly Calendar details should return to horizontal mode above the compact threshold",
                )
            finally:
                WindowLifecycleMixin._schedule_startup_refresh = original_schedule_startup_refresh
                WindowLifecycleMixin._start_lazy_warmup = original_start_lazy_warmup
...
if __name__ == "__main__":
    ...
    test_calendar_startup_defaults_to_reference_today()
    test_calendar_monthly_responsive_layout()

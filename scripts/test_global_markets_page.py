...
def _payload() -> dict[str, Any]:
    rows = [
        ...
        build_global_market_row(_index_config("TAIEX"), _series([21000, 21150, 21400, 21650], ["2024-12-31", "2025-01-02", "2025-01-08", "2025-01-10"]), _timing(1, 5, last_tick_hour=4, exchange_timezone="Asia/Taipei")),
...
def _assert_global_map_labels_clear(width: int, height: int) -> None:
    ...
    assert any(item["country"] == "Taiwan" and item["label"] == "TAIEX" for item in layout)
    for rect in rects:
...
def _build_window():
    ...
    original_global_on_show = GlobalPageMixin._p26_on_show
    original_global_init = GlobalPageMixin.init_page26

    def _init_global_page_with_test_state(self: Any) -> None:
        self.global_page_state = _normalize_global_page_settings({"interval_label": "YTD"})
        original_global_init(self)

    WindowLifecycleMixin._schedule_startup_refresh = lambda self: None
    WindowLifecycleMixin._start_lazy_warmup = lambda self: None
    GlobalPageMixin._p26_on_show = lambda self: None
    GlobalPageMixin.init_page26 = _init_global_page_with_test_state
    try:
        ...
        GlobalPageMixin._p26_on_show = original_global_on_show
        GlobalPageMixin.init_page26 = original_global_init
    return app, window
...
def test_global_page_smoke() -> None:
    ...
    try:
        ...
        assert window.p26_export_2x_btn.text() == "Export 2x Upscale"
        assert "clipboard" in window.p26_export_2x_btn.toolTip().lower()
        assert window.stacked_widget.currentIndex() == 0
        assert window.btn_page1.isChecked()
        assert window._page_initialized(index=25)
        ...
        assert countries.count("Taiwan") == 1
        ...
        assert "Taiwan Capitalization Weighted Stock Index" in export
        assert "^TWII" in export
        assert "Nifty" in export
        assert "SSEC" not in export
        assert "Sensex" not in export
        assert "^GSPTSE" in export
        window.p26_map.resize(720, 480)
        app.processEvents()
        image = window._p26_render_map_image()
        assert not image.isNull()
        assert image.width() == 1440
        assert image.height() == 960
        window.p26_map.resize(720, 480)
        window._p26_export_map_2x()
        clipboard_image = app.clipboard().image()
        assert clipboard_image.width() == 1440
        assert clipboard_image.height() == 960
        assert "1440x960" in window.p26_status_label.text()

        loaded_payload = window._p26_payload
        sentinel = QImage(2, 2, QImage.Format.Format_ARGB32_Premultiplied)
        sentinel.fill(0)
        app.clipboard().setImage(sentinel)
        window._p26_payload = {}
        window._p26_export_map_2x()
        assert app.clipboard().image().size() == sentinel.size()
        assert "Load global index data" in window.p26_status_label.text()
        window._p26_payload = loaded_payload

        map_widget = window.p26_map
        window.p26_map = None
        window._p26_export_map_2x()
        assert "map is not ready" in window.p26_status_label.text()
        window.p26_map = map_widget
        ...
        taiwan_label = next(item for item in layout if item["country"] == "Taiwan" and item["label"] == "TAIEX")
        assert sp_label["show_status"] is True
        assert taiwan_label["show_status"] is True

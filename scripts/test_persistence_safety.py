...
def main() -> None:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            user_data_file.write_bytes(b'\xff\xfe\x00broken-json')
            loaded = persistence._load_user_data_document()
            _assert(isinstance(loaded, dict), 'invalid UTF-8 user data should fall back to defaults')
            backups = list(corrupt_backup_dir.glob('user_data_invalid_json_*.json'))
            _assert(len(backups) == 2, 'invalid UTF-8 user data should also be backed up')

            malformed = persistence._normalize_user_data_document({
                'portfolios': {
                    'portfolio_1': {
                        'portfolio': 5,
                        'chart_slots': 5,
            ...
            malformed_portfolio = malformed['portfolios']['portfolio_1']
            _assert(malformed_portfolio['portfolio'] == [], 'invalid portfolio rows should fall back to an empty list')
            _assert(
                malformed_portfolio['chart_slots'] == persistence.DEFAULT_CHART_SLOTS,
                'invalid chart slots should fall back to defaults',
            )

...
def test_manager_summaries_and_network_free_open() -> None:
    ...
    try:
        ...
        assert window._p4_portfolio_manager_value_text(first) == "Est. $1,030.00"

        second = window._p4_portfolio_manager_summary("portfolio_2")
        assert second["stock_value"] == 330.0
        assert second["options_value"] == 200.0
        assert window._p4_portfolio_manager_value_text(second) == "Est. $780.00"
        ...
        assert window._p4_portfolio_manager_value_text(combined) == "Est. $1,810.00"

        window.last_data = {"portfolio": {"AAPL": {"price": 110.0}}}
        partial = window._p4_portfolio_manager_summary("portfolio_1")
        assert window._p4_portfolio_manager_value_text(partial) == "Partial est. $720.00  ·  1/2 priced"

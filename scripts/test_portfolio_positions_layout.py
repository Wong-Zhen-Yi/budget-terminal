...
def test_positions_layout_and_table_fit() -> None:
    ...
    try:
        ...
        _assert(window.p4_margin_label.text() == "MARGIN", "Margin should use the requested summary label")
        _assert(window.p4_margin_input.value() == 0.0, "Margin should default to zero")
        _assert(
            window.p4_cash_chip.geometry().x()
            < window.p4_margin_chip.geometry().x()
            < window.p4_stock_pl_label.geometry().x(),
            "Margin should sit directly after Brokerage Cash and before the existing summary badges",
        )

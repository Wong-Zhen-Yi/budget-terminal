...
def test_recurring_scheduler_service_initialization_does_not_block_ui_thread() -> None:
    ...
    class _SlowService:
        ...
        def run_due(self, *, catch_up=False):
            run_calls.append(catch_up)
            return {"claimed": 0}
    ...
    try:
        ...
        assert probe._recurring_scheduler_catch_up_pending is False

        timer_instances[0].timeout.callback()
        assert probe._invoke_main.ready.wait(2.0)
        probe.drain_main_callbacks()
        assert run_calls == [True, False]

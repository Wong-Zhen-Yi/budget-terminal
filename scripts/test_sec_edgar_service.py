...
def test_normalization_and_bundle_contract() -> None:
    ...
    quarterly_cashflow = frames["quarterly_cashflow"]
    assert quarterly_cashflow.loc["Operating Cash Flow", "2024-06-30"] == 19.0
    assert quarterly_cashflow.loc["Operating Cash Flow", "2024-09-30"] == 20.0
    assert normalized["provenance"]["Operating Cash Flow"]["quarterly"]["2024-06-30"]["derived"] is False
    ...
    cumulative_q2 = _entry(
        ...
        fp="Q2",
        fy=2024,
        accession="cash-q2-ytd",
    )
    cumulative_q3 = _entry(
        57.0,
        start="2024-01-01",
        end="2024-09-30",
        filed="2024-11-01",
        ...
        fp="Q3",
        fy=2024,
        accession="cash-q3-ytd",
    )
    cumulative_annual = _entry(
        80.0,
        start="2024-01-01",
        end="2024-12-31",
        filed="2025-02-01",
    ...
    cumulative_only = _normalize_company_facts({
        "facts": {
            "us-gaap": {
                "NetCashProvidedByUsedInOperatingActivities": {
                    "units": {
                        "USD": [cumulative_q1, cumulative_q2, cumulative_q3, cumulative_annual]
                    }
                }
            }
        }
    })
    cumulative_cashflow = cumulative_only["frames"]["quarterly_cashflow"]
    assert list(cumulative_cashflow.loc["Operating Cash Flow"]) == [18.0, 19.0, 20.0, 23.0]
    cumulative_provenance = cumulative_only["provenance"]["Operating Cash Flow"]["quarterly"]
    assert cumulative_provenance["2024-06-30"]["derived"] is True
    assert cumulative_provenance["2024-06-30"]["derived_from"] == ["cash-q1", "cash-q2-ytd"]
    assert cumulative_provenance["2024-09-30"]["start"] == "2024-07-01"
    assert cumulative_provenance["2024-12-31"]["form"] == "10-K derived Q4"

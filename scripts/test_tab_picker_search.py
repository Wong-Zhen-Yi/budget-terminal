...
def test_default_navigation_order_and_normalization() -> None:
    ...
    assert migrated["page_order"].index(39) == migrated["page_order"].index(19) + 1
    assert migrated["page_order"].index(29) == migrated["page_order"].index(39) + 1

    retired_scanner_order = [
        *[page_index for page_index in EXPECTED_DEFAULT_NAVIGATION_ORDER if page_index not in {39, 17}],
        38,
        39,
        17,
    ]
    migrated = normalize_navigation_settings({"page_order": retired_scanner_order, "hidden_pages": [38]})
    assert 38 not in migrated["page_order"] and 38 not in migrated["hidden_pages"]
    assert migrated["page_order"].index(39) == migrated["page_order"].index(19) + 1
...
def test_tab_picker_indexes_visible_pages_and_subpages() -> None:
    ...
    try:
        ...
        assert "Signal Scanner" not in labels
        assert "Signal Scanner 2" not in labels
        assert "Signals" in labels
        assert labels.index("Signals") == labels.index("Trading Volumes") + 1
        assert not window.btn_page38.isHidden()
        assert not hasattr(window, "btn_page39")
        assert not window.btn_page40.isHidden()
